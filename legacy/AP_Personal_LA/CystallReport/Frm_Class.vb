Public Class Frm_Class
    ...
    Private Sub Frm_Location_ForCheck_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        fg.FormatString = "ລດ |<ລະຫັດ |<ລະຫັດ        |<ຊື່                            "
    ...
    Private Sub AutoNumber()
        ...
        Call LoadRs("SELECT top 1 CL_ID from Class    Order by CL_ID DESC", VIOT)
        If VIOT.RecordCount <> 0 Then
            VIOTNEW = Format(Val(Mid(VIOT.Fields("CL_ID").Value, 1, 2)) + 1, "0")
        Else
            VIOTNEW = "1"
    ...
    End Sub
    Public Sub save()
        Call LoadRs("SELECT * FROM Class WHERE CL_ID = '" & Txt_ID.Text & "'  ", rs)
        With rs
            If .RecordCount = 0 Then
                Conn.Execute("INSERT INTO Class (CL_ID,CL_No,CL_Nm) " & _
                   " VALUES(" & CDbl(Txt_ID.Text) & "," & _
                   " N'" & (txtno.Text.Trim) & "'," & _
                   " N'" & Txt_name.Text & "')")
            Else
                Conn.Execute("UPDATE Class SET " & _
                   " CL_ID=" & CDbl(Txt_ID.Text) & ", " & _
                  " CL_No=N'" & (txtno.Text) & "', " & _
                   " CL_Nm=N'" & Txt_name.Text & "' " & _
                   " WHERE CL_ID= '" & (Txt_ID.Text) & "'  ")
            End If
        End With
    ...
    Public Sub LoadData()
        fg.Rows = 1
        With rs
            Call LoadRs("SELECT   * from Class order by  CL_ID", rs)
            If .RecordCount > 0 Then
                While Not .EOF()
                    fg.AddItem(.AbsolutePosition & _
                    Chr(9) & (.Fields("CL_ID").Value.ToString) & _
                    Chr(9) & (.Fields("CL_No").Value.ToString) & _
                    Chr(9) & (.Fields("CL_Nm").Value.ToString))
                    .MoveNext()
                End While
            End If
        End With
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການນີ້: " & Trim(fg.get_TextMatrix(fg.Row, 1)) & "  ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From Class Where  CL_ID=N'" & Trim(fg.get_TextMatrix(fg.Row, 1)) & "' ")
            Call LoadData()
        End If
