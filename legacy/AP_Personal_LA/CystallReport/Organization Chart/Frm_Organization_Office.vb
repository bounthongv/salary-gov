Public Class Frm_Organization_Office
    ...
    Private Sub Frm_Location_ForCheck_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Conn.Execute("  update RPT_Organization_Office set RPT_Organization_Office.P_all  = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=002  )  " & _
                   " update RPT_Organization_Office set RPT_Organization_Office.Man   = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.gender =N'ຊາຍ' and ap_cv.Sections_id=002  )   " & _
                    " update RPT_Organization_Office set RPT_Organization_Office.Women   = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.gender =N'ຍິງ'  and ap_cv.Sections_id=002  )   " & _
                    " update RPT_Organization_Office set RPT_Organization_Office.Eak = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=002  and ap_cv.study_ID=01)   " & _
                    " update RPT_Organization_Office set RPT_Organization_Office.tho = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=002  and ap_cv.study_ID=03)   " & _
                    " update RPT_Organization_Office set RPT_Organization_Office.tee = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=002  and ap_cv.study_ID=05)   " & _
                " update RPT_Organization_Office set RPT_Organization_Office.Soung  = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=002  and ap_cv.study_ID=06)   " & _
             " update RPT_Organization_Office set RPT_Organization_Office.kang  = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=002  and ap_cv.study_ID=07)  ")

        Dim rs As New ADODB.Recordset


        Call LoadRs("SELECT  * from RPT_Organization_Office ", rs)
        With rs
    ...
    Public Sub LoadData()
        ...
        With RSC
            ...
            aa = "SELECT    AP_CV.*, AP_Village.Vl_nm, AP_District.Dt_id, AP_District.Dt_nm, AP_Province.PV_ID, AP_Province.PV_nm, " & _
                   ...
                   "   where 1=1 and  AP_CV.Sections_id='002'  ORDER BY   AP_CV.E_id   "
            Call LoadRs(aa, RSC)
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        With rs1
            ...
            If .RecordCount <> 0 Then
                While Not .EOF()
                    'ໂຄງການ HMTA
                    If Trim((.Fields("Sections_id").Value).ToString) = "002" And Trim((.Fields("Department_id").Value).ToString) = "002-001" Then
                        s1_no = s1_no + 1 & vbCr
                        s1 = s1 & s1_no & "/ " & Trim((.Fields("Name_L").Value).ToString) & " " & Trim((.Fields("job_lut").Value).ToString) & vbCr
                    End If
                    'ພະແນກແຜນການ, ສະຖິຕິ ແລະ ສັງລວມ
                    If Trim((.Fields("Sections_id").Value).ToString) = "002" And Trim((.Fields("Department_id").Value).ToString) = "002-002" Then
                        s2_no = s2_no + 1 & vbCr
                        s2 = s2 & s2_no & "/ " & Trim((.Fields("Name_L").Value).ToString) & " " & Trim((.Fields("job_lut").Value).ToString) & vbCr
                     
                    End If
                    'ພະແນກເລຂານຸການ ແລະ ພິທີການ
                    If Trim((.Fields("Sections_id").Value).ToString) = "002" And Trim((.Fields("Department_id").Value).ToString) = "002-003" Then
                        s3_no = s3_no + 1 & vbCr
                        s3 = s3 & s3_no & "/ " & Trim((.Fields("Name_L").Value).ToString) & " " & Trim((.Fields("job_lut").Value).ToString) & vbCr
                    End If
                    'ພະແນກຄຸ້ມຄອງບໍລິຫານ
                    If Trim((.Fields("Sections_id").Value).ToString) = "002" And Trim((.Fields("Department_id").Value).ToString) = "002-004" Then
                        s4_no = s4_no + 1 & vbCr
                        s4 = s4 & s4_no & "/ " & Trim((.Fields("Name_L").Value).ToString) & " " & Trim((.Fields("job_lut").Value).ToString) & vbCr
                    End If
                    'ພະແນກຮ່ວມມື ແລະ ການລົງທຶນ
                    If Trim((.Fields("Sections_id").Value).ToString) = "002" And Trim((.Fields("Department_id").Value).ToString) = "002-005" Then
                        s5_no = s5_no + 1 & vbCr
                        s5 = s5 & s5_no & "/ " & Trim((.Fields("Name_L").Value).ToString) & " " & Trim((.Fields("job_lut").Value).ToString) & vbCr
                    End If
                    'ພະແນກບັນຊີ-ການເງິນ
                    If Trim((.Fields("Sections_id").Value).ToString) = "002" And Trim((.Fields("Department_id").Value).ToString) = "002-006" Then
                        s6_no = s6_no + 1 & vbCr
                        s6 = s6 & s6_no & "/ " & Trim((.Fields("Name_L").Value).ToString) & " " & Trim((.Fields("job_lut").Value).ToString) & vbCr
                    End If
                    'ພະແນກສົ່ງເສີມຄວາມກ້າວໜ້າຂອງແມ່ຍິງ
                    If Trim((.Fields("Sections_id").Value).ToString) = "002" And Trim((.Fields("Department_id").Value).ToString) = "002-007" Then
                        s7_no = s7_no + 1 & vbCr

                        s7 = s7 & s7_no & "/ " & Trim((.Fields("Name_L").Value).ToString) & " " & Trim((.Fields("job_lut").Value).ToString) & vbCr
                    End If
                    .MoveNext()
                End While
        ...
        aa = "    update RPT_Organization_Office set RPT_Organization_Office.office_nm1 = AP_CV .Name_L from ap_cv  where ap_cv.Sections_id=002  and ap_cv.job_lut_ID=006 " & _
      "  update RPT_Organization_Office set RPT_Organization_Office.office_nm2 = AP_CV .Name_L from ap_cv  where ap_cv.Sections_id=002 and ap_cv.job_lut_ID=011  and ap_cv.txthong=1 " & _
       " update RPT_Organization_Office set RPT_Organization_Office.office_nm3 = AP_CV .Name_L from ap_cv  where ap_cv.Sections_id=002 and ap_cv.job_lut_ID=011  and ap_cv.txthong=2   " & _
     "   update RPT_Organization_Office set RPT_Organization_Office.office_nm4 = AP_CV .Name_L from ap_cv  where ap_cv.Sections_id=002 and ap_cv.job_lut_ID=011   and ap_cv.txthong=3 " & _
      "  update RPT_Organization_Office set RPT_Organization_Office.office_nm5 = AP_CV .Name_L from ap_cv  where ap_cv.Sections_id=002 and ap_cv.job_lut_ID=011   and ap_cv.txthong=4 "
        Conn.Execute(aa)

        With RSC
            aa = "select N'" & s1 & "' as s1 ,N'" & s2 & "' as s2 ,N'" & s3 & "' as s3 ,N'" & s4 & "' as s4 ,N'" & s5 & "' as s5 ,N'" & s6 & "' as s6 ,N'" & s7 & "' as s7   , *  from RPT_Organization_Office  "
            Call LoadRs(aa, RSC)
            If .RecordCount = 0 Then MsgBox("Data empty", vbInformation, "Check") : Exit Sub
            Dim Frm As New FrmPreview
            Dim Rpt As New Report_Organization_Office
            '=========================================================
    ...
    Private Sub Button3_EnabledChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button3.EnabledChanged
