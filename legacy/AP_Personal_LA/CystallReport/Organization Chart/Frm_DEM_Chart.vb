Public Class Frm_DEM_Chart
    Dim rs As New ADODB.Recordset

    Private Sub Frm_DEM_Chart_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Conn.Execute("  update RPT_DEM set RPT_DEM.P_all  = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=008  )  " & _
                   " update RPT_DEM set RPT_DEM.Man   = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.gender =N'ຊາຍ' and ap_cv.Sections_id=008  )   " & _
                    " update RPT_DEM set RPT_DEM.Women   = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.gender =N'ຍິງ'  and ap_cv.Sections_id=008  )   " & _
                    " update RPT_DEM set RPT_DEM.Eak = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=008  and ap_cv.study_ID=01)   " & _
                    " update RPT_DEM set RPT_DEM.tho = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=008  and ap_cv.study_ID=03)   " & _
                    " update RPT_DEM set RPT_DEM.tee = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=008  and ap_cv.study_ID=05)   " & _
                " update RPT_DEM set RPT_DEM.Soung  = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=008  and ap_cv.study_ID=06)   " & _
             " update RPT_DEM set RPT_DEM.kang  = ( select COUNT (AP_CV .Name_L) from ap_cv  where ap_cv.Sections_id=008  and ap_cv.study_ID=07)  ")

        Dim rs As New ADODB.Recordset


        Call LoadRs("SELECT  * from RPT_DEM ", rs)
        With rs
    ...
    Public Sub LoadData()
        ...
        With RSC
            ...
            aa = "SELECT    AP_CV.*, AP_Village.Vl_nm, AP_District.Dt_id, AP_District.Dt_nm, AP_Province.PV_ID, AP_Province.PV_nm, " & _
                   ...
                   "   where 1=1 and  AP_CV.Sections_id='008'  ORDER BY   AP_CV.E_id   "
            Call LoadRs(aa, RSC)
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        With rs1
            ...
            If .RecordCount <> 0 Then
                While Not .EOF()
                    ...
                    If Trim((.Fields("Sections_id").Value).ToString) = "008" And Trim((.Fields("Department_id").Value).ToString) = "008-001" Then
                        s1_no = s1_no + 1 & vbCr
                        s1 = s1 & s1_no & "/ " & Trim((.Fields("Name_L").Value).ToString) & " " & Trim((.Fields("job_lut").Value).ToString) & vbCr
                    End If
                    'ພະແນກກວດກາພັກ
                    If Trim((.Fields("Sections_id").Value).ToString) = "008" And Trim((.Fields("Department_id").Value).ToString) = "008-002" Then
                        s2_no = s2_no + 1 & vbCr
                    ...
                    If Trim((.Fields("Sections_id").Value).ToString) = "008" And Trim((.Fields("Department_id").Value).ToString) = "008-003" Then
                        s3_no = s3_no + 1 & vbCr
                        s3 = s3 & s3_no & "/ " & Trim((.Fields("Name_L").Value).ToString) & " " & Trim((.Fields("job_lut").Value).ToString) & vbCr
                    End If
                    'ພະແນກນິຕິກຳ
                    If Trim((.Fields("Sections_id").Value).ToString) = "008" And Trim((.Fields("Department_id").Value).ToString) = "008-004" Then
                        s4_no = s4_no + 1 & vbCr
        ...
        aa = "    update RPT_DEM set RPT_DEM.office_nm1 = AP_CV .Name_L from ap_cv  where ap_cv.Sections_id=008  and ap_cv.job_lut_ID=003 " & _
      "  update RPT_DEM set RPT_DEM.office_nm2 = AP_CV .Name_L from ap_cv  where ap_cv.Sections_id=008 and ap_cv.job_lut_ID=008  and ap_cv.txthong=1 " & _
       " update RPT_DEM set RPT_DEM.office_nm3 = AP_CV .Name_L from ap_cv  where ap_cv.Sections_id=008 and ap_cv.job_lut_ID=008 and ap_cv.txthong=2 "
        Conn.Execute(aa)

        With RSC
            aa = "select N'" & s1 & "' as s1 ,N'" & s2 & "' as s2 ,N'" & s3 & "' as s3 ,N'" & s4 & "' as s4    , *  from RPT_DEM  "
            Call LoadRs(aa, RSC)
            If .RecordCount = 0 Then MsgBox("Data empty", vbInformation, "Check") : Exit Sub
            Dim Frm As New FrmPreview
            Dim Rpt As New Report_DEM_Chart
            '=========================================================
