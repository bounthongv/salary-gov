...
Public Class Frm_Salary_bank_Report
    Dim Sql As String
...
" and   month(AP_Salary_in_Month.AtMonth) ='" & Month(DT_Month.Value) & "' and year(AP_Salary_in_Month.AtMonth) ='" & Year(DT_Month.Value) & "' and AP_CV.Bank_no<>'' order by   AP_CV.order_no  "
                Call LoadRs(aa, RSC)
                If .RecordCount = 0 Then MsgBox("Data empty", vbInformation, "Check") : Exit Sub
                Dim Frm As New FrmPreview
                Dim Rpt As New Report_Salary_In_bank
...
"    Department ON AP_CV.Department_id = Department.DP_ID INNER JOIN      " & _
      ...
      "      job ON AP_CV.duties_Id = job.job_id  WHERE 1=1  " & Sql & " " & shr_Class_vel & " " & shr_Department & " " & shr_section & " " & shr_job_phuk & " " & shr_job_lut & "" & Shr_persen & " " & _
          " and   month(AP_Salary_in_Month.AtMonth) ='" & Month(DT_Month.Value) & "' and year(AP_Salary_in_Month.AtMonth) ='" & Year(DT_Month.Value) & "' and AP_CV.Bank_no='' order by   AP_CV.order_no  "
                ...
                If .RecordCount = 0 Then MsgBox("Data empty", vbInformation, "Check") : Exit Sub
                Dim Frm As New FrmPreview
                Dim Rpt As New Report_Salary_no_bank
                Dim myTextObjectOnReport As CrystalDecisions.CrystalReports.Engine.TextObject
                'myTextObjectOnReport = CType(Rpt.ReportDefinition.ReportObjects.Item("Text13"), CrystalDecisions.CrystalReports.Engine.TextObject)
                'myTextObjectOnReport.Text = "(ຂອງ " & Location_nm & ")"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If RadioButton1.Checked = True Then
            ...
            "   AP_Salary_in_Month.Total_Money_curr_Exing, AP_CV.DOB, " & _
        ...
        " AP_Salary_in_Month INNER JOIN       AP_CV ON AP_Salary_in_Month.PersonID = AP_CV.E_ID INNER JOIN    " & _
             " AP_Sections ON AP_CV.Sections_id = AP_Sections.Sec_id INNER JOIN       " & _
                " Department ON AP_CV.Department_id = Department.DP_ID INNER JOIN        " & _
                  "  Type_In ON AP_CV.type_in_id = Type_In.In_ID  INNER JOIN   " & _
                     "   job ON AP_CV.duties_Id = job.job_id  WHERE 1=1  " & Sql & " " & shr_Class_vel & " " & shr_Department & " " & shr_section & " " & shr_job_phuk & " " & shr_job_lut & "" & Shr_persen & " " & _
           " and   month(AP_Salary_in_Month.AtMonth) ='" & Month(DT_Month.Value) & "' and year(AP_Salary_in_Month.AtMonth) ='" & Year(DT_Month.Value) & "' and AP_CV.Bank_no<>'' order by  Department.DP_ID,AP_CV.order_no   "
                    Call LoadRs(aa, RSC)
                    If .RecordCount = 0 Then MsgBox("Data empty", vbInformation, "Check") : Exit Sub
                    ...
                    xlWorkSheet.Cells(1, 8).Value = "ຈຳນວນເງິນ     "
                    xlWorkSheet.Cells(1, 9).Value = "ວັນເດືອນປີເກີດ "
                    xlWorkSheet.Cells(1, 10).Value = "ເບີໂທ    "
                    xlWorkSheet.Cells(1, 11).Value = "ສະຖານະ"
                    ...
                    xlWorkSheet.SaveAs("" & Trim(txtpart.Text) & "\ສະເພາະພະນັກງານທີ່ມີເລກບັນຊີ.xlsx")
        ...
        Else
            ...
            Try
                With RSC
                    aa = "SELECT str( month(AP_Salary_in_Month.AtMonth))+'/'+str(year(AP_Salary_in_Month.AtMonth)), AP_Sections.Sec_nmL,Department.DP_Name ,job.job_nm ,AP_CV.Name_L,AP_CV.Name_E, " & _
           "   AP_Salary_in_Month.Total_Money_curr_Exing, AP_CV.DOB, " & _
          "  AP_CV.Phone,    Type_In.In_nm  FROM          " & _
       " AP_Salary_in_Month INNER JOIN       AP_CV ON AP_Salary_in_Month.PersonID = AP_CV.E_ID INNER JOIN    " & _
            " AP_Sections ON AP_CV.Sections_id = AP_Sections.Sec_id INNER JOIN       " & _
          ...
          " and   month(AP_Salary_in_Month.AtMonth) ='" & Month(DT_Month.Value) & "' and year(AP_Salary_in_Month.AtMonth) ='" & Year(DT_Month.Value) & "' and AP_CV.Bank_no='' order by AP_CV.order_no   "
                    ...
                    xlWorkSheet.Cells(1, 5).Value = "ຊື່ ແລະ ນາມສະກຸນ (ລາວ)"
                    xlWorkSheet.Cells(1, 6).Value = "ຊື່ ແລະ ນາມສະກຸນ (ອັດກິດ)"
                    xlWorkSheet.Cells(1, 7).Value = "ຈຳນວນເງິນ     "
                    xlWorkSheet.Cells(1, 8).Value = "ວັນເດືອນປີເກີດ "
                    xlWorkSheet.Cells(1, 9).Value = "ເບີໂທ    "
                    xlWorkSheet.Cells(1, 10).Value = "ສະຖານະ"
                    ...
                    xlWorkSheet.SaveAs("" & Trim(txtpart.Text) & "\ສະເພາະພະນັກງານທີ່ເບີກເງີນສົດ.xlsx")
