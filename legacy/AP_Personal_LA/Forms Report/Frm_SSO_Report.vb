...
Public Class Frm_SSO_Report
    Dim Sql As String
    Private Sub Frm_SSO_Report_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Sql = ""
        shr_Class_vel = ""
        shr_Department = ""
        shr_section = ""
...
" and   month(AP_Salary_in_Month.AtMonth) ='" & Month(DT_Month.Value) & "' and year(AP_Salary_in_Month.AtMonth) ='" & Year(DT_Month.Value) & "' and AP_Salary_in_Month.txtpeple='1' and AP_Salary_in_Month.Employee_LAK>0  order by   AP_CV.order_no   "
                Call LoadRs(aa, RSC)
                If .RecordCount = 0 Then MsgBox("Data empty", vbInformation, "Check") : Exit Sub
                Dim Frm As New FrmPreview
                Dim Rpt As New Report_Social_Money
                ...
                myTextObjectOnReport = CType(Rpt.ReportDefinition.ReportObjects.Item("Text5"), CrystalDecisions.CrystalReports.Engine.TextObject)
                ...
                'myText2.Text = (Format(DT_Month.Value, "MM/yyyy"))
            ...
            Else
              ...
              "    Type_In ON AP_CV.type_in_id = Type_In.In_ID  INNER JOIN " & _
               "      job ON AP_CV.duties_Id = job.job_id  WHERE 1=1  " & Sql & " " & shr_Class_vel & " " & shr_Department & " " & shr_section & " " & shr_job_phuk & " " & shr_job_lut & "" & Shr_persen & " " & _
              " and   month(AP_Salary_in_Month.AtMonth) ='" & Month(DT_Month.Value) & "' and year(AP_Salary_in_Month.AtMonth) ='" & Year(DT_Month.Value) & "' and AP_Salary_in_Month.txtpeple='2' and AP_Salary_in_Month.Employee_LAK>0  order by   AP_CV.order_no   "
                Call LoadRs(aa, RSC)
                If .RecordCount = 0 Then MsgBox("Data empty", vbInformation, "Check") : Exit Sub
                Dim Frm As New FrmPreview
                Dim Rpt As New Report_Social_Money_EX
                Dim myTextObjectOnReport As CrystalDecisions.CrystalReports.Engine.TextObject
                'myTextObjectOnReport = CType(Rpt.ReportDefinition.ReportObjects.Item("Text13"), CrystalDecisions.CrystalReports.Engine.TextObject)
                'myTextObjectOnReport.Text = "(ຂອງ " & Location_nm & ")"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If RadioButton1.Checked = True Then
            ...
            Try
                With RSC
                    aa = "SELECT    AP_CV.SSO_no, AP_Salary_in_Month.Name_L, AP_Salary_in_Month.Salary,'2000,000', " & _
                            " AP_Salary_in_Month.Employer_LAK , AP_Salary_in_Month.Employee_LAK ,  " & _
                             "   AP_Salary_in_Month.Employer_LAK + AP_Salary_in_Month.Employee_LAK " & _
    ...
    " and   month(AP_Salary_in_Month.AtMonth) ='" & Month(DT_Month.Value) & "' and year(AP_Salary_in_Month.AtMonth) ='" & Year(DT_Month.Value) & "' and AP_Salary_in_Month.txtpeple='1' and AP_Salary_in_Month.Employee_LAK>0  order by AP_CV.order_no   "
                    ...
                    xlWorkSheet.Cells(1, 1).Value = "ລະຫັດ ປ/ສ"
                    xlWorkSheet.Cells(1, 2).Value = "ລາຍຊື່ຜູ້ອອກແຮງງານ"
                    xlWorkSheet.Cells(1, 3).Value = "ເງິນເດືອນພື້ນຖານ"
                    xlWorkSheet.Cells(1, 4).Value = "ເພດານເງິນສ/ທ"
                    xlWorkSheet.Cells(1, 5).Value = "ຜູ່ໃຊ້ແຮງງານ 5%"
                    xlWorkSheet.Cells(1, 6).Value = "ຜູ່ໃຊ້ແຮງງານ 4.5% "
                    xlWorkSheet.Cells(1, 7).Value = "ລວມ"
                    ...
                    xlWorkSheet.SaveAs("" & Trim(txtpart.Text) & "\ເງິນສົມທົບເຂົ້າອົງກອນປະກັນສັງຄົມ.xlsx")
                    xlWorkBook.Close()
                    xlApp.Quit()
                    releaseObject(xlApp)
    ...
    " and   month(AP_Salary_in_Month.AtMonth) ='" & Month(DT_Month.Value) & "' and year(AP_Salary_in_Month.AtMonth) ='" & Year(DT_Month.Value) & "' and AP_Salary_in_Month.txtpeple='2' and AP_Salary_in_Month.Employee_LAK>0  order by AP_CV.order_no   "
                    ...
                    xlWorkSheet.Cells(1, 3).Value = "ເງິນເດືອນພື້ນຖານ"
                    xlWorkSheet.Cells(1, 4).Value = "ເພດານເງິນສ/ທ"
                    xlWorkSheet.Cells(1, 5).Value = "ຜູ່ໃຊ້ແຮງງານ 10%"
                    xlWorkSheet.Cells(1, 6).Value = "ຜູ່ໃຊ້ແຮງງານ 10% "
                    xlWorkSheet.Cells(1, 7).Value = "ລວມ"
                    ...
                    ' Save as path of excel sheet
                    'mStrFTP = mStrFTP & "/" & Trim(txtpic1_nm.Text)
                    xlWorkSheet.SaveAs("" & Trim(txtpart.Text) & "\ເງິນສົມທົບເຂົ້າອົງກອນປະກັນສັງຄົມ (ຄນຄ່າງປະເທດ).xlsx")
                    xlWorkBook.Close()
                    xlApp.Quit()
                    releaseObject(xlApp)
                    releaseObject(xlWorkBook)
