...
Public Class Frm_Salary_Report
    Dim Sql As String
    Private Sub Frm_Salary_Report_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Sql = ""
        shr_Class_vel = ""
        shr_Department = ""
        shr_section = ""
    ...
    " and   month(AP_Salary_in_Month.AtMonth) ='" & Month(DT_Month.Value) & "' and year(AP_Salary_in_Month.AtMonth) ='" & Year(DT_Month.Value) & "' order by AP_CV.order_no   "
            ...
            If Chk_em_In.Checked = True Then
                ...
                Dim Rpt As New Report_Salary_In_Month_in_new
                ...
                myTextObjectOnReport = CType(Rpt.ReportDefinition.ReportObjects.Item("Text27"), CrystalDecisions.CrystalReports.Engine.TextObject)
                myTextObjectOnReport.Text = "ບັນຊີເບີກຈ່າຍເງີນເດືອນຂອງ " & cmb_Department.Text & " ລັດວິສາຫະກິດການບິນລາວ "
            ...
            Else
                ...
                myTextObjectOnReport.Text = "ບັນຊີເບີກຈ່າຍເງີນເດືອນຂອງພະນັກງານ " & Cmb_Sections.Text & ""
                myTextObjectOnReport = CType(Rpt.ReportDefinition.ReportObjects.Item("Text13"), CrystalDecisions.CrystalReports.Engine.TextObject)
                myTextObjectOnReport.Text = (Format(DT_Month.Value, "MM/yyyy"))
                'If chk_section.Checked = True Then
                '    myTextObjectOnReport = CType(Rpt.ReportDefinition.ReportObjects.Item("Text27"), CrystalDecisions.CrystalReports.Engine.TextObject)
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If Chk_em_In.Checked = True Then
            ...
            Try
                With RSC
                    aa = "SELECT     AP_Salary_in_Month.Name_L, job.job_nm, AP_Salary_in_Month.Salary, AP_Salary_in_Month.txtTum_money, AP_Salary_in_Month.WDayOfMonth, AP_Salary_in_Month.txt_H_oertime, " & _
                "  AP_Salary_in_Month.total_amount- AP_Salary_in_Month.txtTum_money, AP_Salary_in_Month.MOvertimeTotal,AP_Salary_in_Month.Sum_Addtional, " & _
                " AP_Salary_in_Month.total_amount + AP_Salary_in_Month.MOvertimeTotal + AP_Salary_in_Month.Employer_LAK + AP_Salary_in_Month.tax_money  AS aa , " & _
                "   AP_Salary_in_Month.Employee_LAK, AP_Salary_in_Month.Employer_LAK, AP_Salary_in_Month.tax_money,   " & _
                 " (AP_Salary_in_Month.total_amount + AP_Salary_in_Month.MOvertimeTotal + AP_Salary_in_Month.Employer_LAK + AP_Salary_in_Month.tax_money) - (AP_Salary_in_Month.Employee_LAK + AP_Salary_in_Month.Employer_LAK + AP_Salary_in_Month.tax_money ) , " & _
                 "   AP_Salary_in_Month.Sum_Deducation,AP_Salary_in_Month.Total_Money_curr_Exing " & _
    ...
    " and   month(AP_Salary_in_Month.AtMonth) ='" & Month(DT_Month.Value) & "' and year(AP_Salary_in_Month.AtMonth) ='" & Year(DT_Month.Value) & "' order by AP_CV.order_no   "
                    ...
                    xlWorkSheet.Cells(1, 1).Value = "ຊື່ ແລະ ນາມສະກຸນ"
                    xlWorkSheet.Cells(1, 2).Value = "ຕຳແໜ່ງ"
                    xlWorkSheet.Cells(1, 3).Value = "ເງິນເດືອນພື້ນຖານ"
                    xlWorkSheet.Cells(1, 4).Value = "ເງີນຕຳແໜ່ງບໍລິຫານ"
                    ...
                    xlWorkSheet.Cells(1, 9).Value = "ເງິນອຸດໜູນເພີ່ມ"
                    xlWorkSheet.Cells(1, 10).Value = "ລວມເງີນເດືອນທັງໝົດ"
                    xlWorkSheet.Cells(1, 11).Value = "ບຸກຄົນ 4.5%"
                    xlWorkSheet.Cells(1, 12).Value = "ບໍລິສັດ 5%"
                    xlWorkSheet.Cells(1, 13).Value = "ອາກອນເງິນເດືອນ"
                    ...
                    xlWorkSheet.SaveAs("" & Trim(txtpart.Text) & "\ບັນຊີເບີກຈ່າຍເງີນເດືອນພາຍໃນ.xlsx")
                    xlWorkBook.Close()
                    xlApp.Quit()
                    releaseObject(xlApp)
        ...
        Else
            ...
            Try
                With RSC
                    aa = "SELECT     AP_Salary_in_Month.Name_L, job.job_nm,AP_Salary_in_Month.SalaryCurrency,AP_Salary_in_Month.rate, AP_Salary_in_Month.Salary, AP_Salary_in_Month.Salary * AP_Salary_in_Month.rate, AP_Salary_in_Month.txtTum_money, AP_Salary_in_Month.WDayOfMonth, AP_Salary_in_Month.txt_H_oertime, " & _
                       " AP_Salary_in_Month.total_amount , AP_Salary_in_Month.MOvertimeTotal ,AP_Salary_in_Month.total_amount + AP_Salary_in_Month.MOvertimeTotal+ AP_Salary_in_Month.txtTum_money , " & _
                     " AP_Salary_in_Month.Sum_Addtional,AP_Salary_in_Month.Sum_Deducation, AP_Salary_in_Month.total_amount + AP_Salary_in_Month.MOvertimeTotal + AP_Salary_in_Month.Sum_Addtional -(AP_Salary_in_Month.Sum_Deducation), " & _
                    "  0, AP_Salary_in_Month.Employee_LAK, AP_Salary_in_Month.Employer_LAK, " & _
                 " AP_Salary_in_Month.total_amount + AP_Salary_in_Month.MOvertimeTotal + AP_Salary_in_Month.Employer_LAK + AP_Salary_in_Month.txtTum_money - (AP_Salary_in_Month.Employee_LAK)  AS aa , " & _
                "  AP_Salary_in_Month.tax_money,   " & _
                 " AP_Salary_in_Month.Sum_Deducation_After / AP_Salary_in_Month.rate,AP_Salary_in_Month.Sum_Deducation_After, " & _
                 "   AP_Salary_in_Month.Total_Money_curr ,AP_Salary_in_Month.Total_Money_curr_Exing " & _
             "   FROM         AP_Salary_in_Month INNER JOIN " & _
                  "    AP_CV ON AP_Salary_in_Month.PersonID = AP_CV.E_ID INNER JOIN " & _
                   "   AP_Sections ON AP_CV.Sections_id = AP_Sections.Sec_id INNER JOIN " & _
                  "    Department ON AP_CV.Department_id = Department.DP_ID INNER JOIN " & _
    ...
    " and   month(AP_Salary_in_Month.AtMonth) ='" & Month(DT_Month.Value) & "' and year(AP_Salary_in_Month.AtMonth) ='" & Year(DT_Month.Value) & "' order by AP_CV.order_no   "
                    ...
                    xlWorkSheet.Cells(1, 1).Value = "ຊື່ ແລະ ນາມສະກຸນ"
                    xlWorkSheet.Cells(1, 2).Value = "ຕຳແໜ່ງ"
                    xlWorkSheet.Cells(1, 3).Value = "ສະກຸນເງິນ"
                    xlWorkSheet.Cells(1, 4).Value = "ອັດຕາແລກປ່ຽນ"
                    xlWorkSheet.Cells(1, 5).Value = "ເງິນເດືອນພື້ນຖານ ອັດຕາຕ່າງປະເທດ"
                    xlWorkSheet.Cells(1, 6).Value = "ເງິນເດືອນພື້ນຖານ ເປັນກິບ"
                    xlWorkSheet.Cells(1, 7).Value = "ເງີນຕຳແໜ່ງບໍລິຫານ"
                    xlWorkSheet.Cells(1, 8).Value = "ຊົວໂມງປົກກະຕິ"
                    xlWorkSheet.Cells(1, 9).Value = "ຊົວໂມງເກີນ  "
                    xlWorkSheet.Cells(1, 10).Value = "ຄິດເປັນເງິນປົກກະຕິ"
                    xlWorkSheet.Cells(1, 11).Value = "ຄິດເປັນເງິນເກີນ "
                    xlWorkSheet.Cells(1, 12).Value = "ເລວມເງີນແຮງງານ"
                    xlWorkSheet.Cells(1, 13).Value = "ເງິນດັດແກ້ເພີ່ມ"
                    xlWorkSheet.Cells(1, 14).Value = "ເງິນດັດແກ້ຫັກ"
                    xlWorkSheet.Cells(1, 15).Value = "ລວມເງີນເດືອນ"
                    xlWorkSheet.Cells(1, 16).Value = "ເງິນຫັກຄັງສະຫວັດດີການ"
                    xlWorkSheet.Cells(1, 17).Value = "ບຸກຄົນ 10% "
                    xlWorkSheet.Cells(1, 18).Value = "ບໍລິສັດ 10% "
                    xlWorkSheet.Cells(1, 19).Value = "ລາຍຮັບຖຶກທີ່ອາກອນ"
                    xlWorkSheet.Cells(1, 20).Value = "ອາກອນເງິນເດືອນ"
                    xlWorkSheet.Cells(1, 21).Value = "ຫັກເງິນຢືມຕາມສະກຸນເງິນ "
                    xlWorkSheet.Cells(1, 22).Value = "ຫັກເງິນຢືມເປັນກິບ "
                    xlWorkSheet.Cells(1, 23).Value = "ຈຳນວນເບິກຈ່າຍຕົວຈິງເປັນກິບ"
                    xlWorkSheet.Cells(1, 24).Value = "ຈຳນວນເບິກຈ່າຍຕົວຈິງຕາມສະກຸນເງິນ"
                    For i = 0 To ds.Tables(0).Rows.Count - 1
                        'Column
                        'xlApp = New Excel.ApplicationClass
                        'xlWorkBook = xlApp.Workbooks.Add(misValue)
                    ...
                    ' Save as path of excel sheet
                    'mStrFTP = mStrFTP & "/" & Trim(txtpic1_nm.Text)
                    xlWorkSheet.SaveAs("" & Trim(txtpart.Text) & "\ບັນຊີເບີກຈ່າຍເງີນເດືອນສາຕ່າງປະເທດ.xlsx")
                    xlWorkBook.Close()
                    xlApp.Quit()
                    releaseObject(xlApp)
                    releaseObject(xlWorkBook)
