Public Class Frm_Mom_List
    ...
    Dim StrSQL, ConString, SUPP, shr_status As String
    Dim P As Integer
    Dim SQl1 As String
    Dim sql2 As String
    Private Sub Button1_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Me.Close()
    End Sub
    Private Sub Frm_Mom_List_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load



        Fg1.FormatString = ">ລ/ດ |<ເລກທີຂໍ້ຕົກລົງ  |^ວັນທີຕົກລົງ  |<ລະຫັດພະນັກງານ|<ຊື່ ແລະ ນາມສະກຸນ |<ຊື່ (ພາສາອັງກິດ)      |<ເບີໂທລະສັບ     |<ບ່ອນປະຈຳການ   |<ພະແນກ        |>ຈຳນວນ|>ລວມເງີນອຸດໜູນ|<ສະຖານະການເບີກຈ່າຍ |<ລະອຽດຂອງລູກທີ່ໄດ້ຮັບເງີນອຸດໜູນ|<ບ້ານຢູ່ປະຈຸບັນ |<ເມືອງ             |<ແຂວງ            "
    ...
    Private Sub loaddata()
        ...
        With RSC
            Dim aa As String
            aa = "SELECT     AP_mom_Money.*,AP_CV.DT_strt_work, AP_CV.start_work,AP_CV.DT_Work_now, AP_CV.txtV_C,  AP_Village.Vl_nm, AP_District.Dt_nm, AP_Province.PV_nm " & _
            " FROM         AP_mom_Money INNER JOIN " & _
          "     AP_CV ON AP_mom_Money.E_ID = AP_CV.E_ID INNER JOIN " & _
          ...
          "     AP_Province ON AP_District.PV_id = AP_Province.PV_ID   where 1=1  " & shr_status & "" & Sql & " " & shr_section & " " & shr_Department & "  " & shr_job_phuk & "   ORDER BY  AP_CV.E_ID "
            Call LoadRs(aa, RSC)
            ...
            If .RecordCount <> 0 Then
                ...
                While Not .EOF()
                    Fg1.AddItem(.AbsolutePosition & _
                              ...
                              Chr(9) & Format(CDate(RSC.Fields("DT_in").Value), "dd/MM/yyyy") & _
                                ...
                                Chr(9) & Trim((RSC.Fields("Name_L").Value).ToString) & _
                                 ...
                                 Chr(9) & Trim((RSC.Fields("Sections").Value).ToString) & _
                                  Chr(9) & Trim((RSC.Fields("Department").Value).ToString) & _
                                    Chr(9) & Trim((RSC.Fields("txtmom").Value).ToString) & " ຄົນ" & _
                                   Chr(9) & Format(CDbl(.Fields("txtmom_Money").Value), "##,##0.00") & " ກີບ" & _
                                     Chr(9) & Trim((RSC.Fields("status_nm").Value).ToString) & _
                                      Chr(9) & Trim((RSC.Fields("txtabount_mom").Value).ToString) & _
    ...
    Private Sub Button9_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnAddNew.Click
        ...
        EditActive = False
        'Frm_Mom.MdiParent = FrmAPInvioce
        'Frm_Mom.WindowState = FormWindowState.Normal
        'Frm_Mom.ShowIcon = False
        Frm_Mom.Show()
    End Sub
    ...
    Private Sub Fg1_MouseUpEvent(ByVal sender As Object, ByVal e As AxVSFlex8U._IVSFlexGridEvents_MouseUpEvent) Handles Fg1.MouseUpEvent
        ...
        Bill_ID = Fg1.get_TextMatrix(Fg1.Row, 1)
        E_ID = Fg1.get_TextMatrix(Fg1.Row, 3)
    ...
    Private Sub Button8_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnEdit.Click
        ...
        EditActive = True
        'Frm_Mom.MdiParent = FrmAPInvioce
        'Frm_Mom.WindowState = FormWindowState.Maximized
        'Frm_Mom.ShowIcon = False
        Frm_Mom.Show()
    End Sub
    Private Sub Button7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDel.Click
        ...
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການ ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
         ...
         " WHERE E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'")
            Conn.Execute(" UPDATE AP_CV SET txtToltal_All=Total_remaining-Tax+khongsep+txtson_money " & _
            " WHERE E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'")
    ...
    " WHERE E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'")
            ...
            Conn.Execute(" UPDATE AP_Salary SET txtToltal_All=Total_remaining-Tax+khongsep+txtson_money " & _
            " WHERE E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'")
            ...
            " txtmom=0" & _
             " WHERE E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'")
            Conn.Execute("Delete From AP_mom_Money Where  bill_no='" & Trim(Fg1.get_TextMatrix(Fg1.Row, 1) & "' and  E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'"))
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        With RSC
            Dim sa As String = "SELECT     AP_mom_Money.*,AP_CV.DT_strt_work, AP_CV.start_work,AP_CV.DT_Work_now, AP_CV.txtV_C,  AP_Village.Vl_nm, AP_District.Dt_nm, AP_Province.PV_nm " & _
            " FROM         AP_mom_Money INNER JOIN " & _
          "     AP_CV ON AP_mom_Money.E_ID = AP_CV.E_ID INNER JOIN " & _
           "    AP_Village ON AP_CV.Add_Vill_ID = AP_Village.Vl_ID INNER JOIN " & _
           "    AP_District ON AP_Village.Dt_id = AP_District.Dt_id INNER JOIN " & _
          "     AP_Province ON AP_District.PV_id = AP_Province.PV_ID   where 1=1  " & shr_status & "" & Sql & " " & shr_section & " " & shr_Department & "  " & shr_job_phuk & "   ORDER BY  AP_CV.E_ID "
            ...
            Dim Rpt As New Report_Mom_list
    ...
    Private Sub txtTdate_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtTdate.ValueChanged
        Sql = " AND AP_mom_Money.DT_in between '" & Format(txtFdate.Value, "MM/dd/yyyy") & "'AND '" & Format(txtTdate.Value, "MM/dd/yyyy") & "'"
        Call loaddata()
    End Sub
    ...
    Private Sub Button6_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button6.Click
        ...
        Dim aa As String
        Dim rs As New ADODB.Recordset
        aa = "SELECT    * from AP_Mom_Money  where bill_no='" & Trim(Fg1.get_TextMatrix(Fg1.Row, 1)) & "' and  E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "' "
        Call LoadRs(aa, rs)
        With rs
            If .RecordCount > 0 Then

                txt_no_in.Text = Trim(.Fields("bill_no").Value.ToString)
                ...
                txtmom_Money.Text = Format(CDbl(.Fields("txtmom_Money").Value), "##,##0.00")
...
" WHERE E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'")
        ...
        Conn.Execute(" UPDATE AP_mom_Money SET " & _
              " status=1," & _
               " Bill_cancel=N'" & txt_no_out.Text & "'," & _
                   " DT_cancel=N'" & Format(DT_out.Value, "yyyy-MM-dd") & "'," & _
              " status_nm=N'ຍົກເລີກການຈ່າຍແລ້ວ'" & _
               " WHERE bill_no= '" & txt_no_in.Text & "' and E_ID= '" & txtid.Text & "'")
    ...
    Private Sub chk_status_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chk_status.CheckedChanged
        If chk_status.Checked = True Then
            ...
            shr_status = " AND AP_mom_Money.status_nm = N'" & cmb_status.Text & "' "
        Else
            cmb_status.Text = ""
            shr_status = ""
