Public Class Frm_EN_Table_List
    ...
    Private Sub Button1_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Me.Close()
    End Sub
    Private Sub Frm_EN_Table_List_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load


        Fg1.FormatString = ">ລ/ດ  |<ກົມ   |<ກົມ                                |<ພະແນກ                      |^ວັນທີ   |<ແຜນຊັບຊ້ອນປີ                        "
        FG2.FormatString = "^ລ/ດ |<ຊື່ ພະແນກແລະກຸ່ມວຽກ                            |>ຄວາມຕ້ອງການທັງໝົດ|>ລວມ |>ບໍລິຫານ |>ວິຊາການ|>ລວມ |>ບໍລິຫານ |>ວິຊາການ|>ແຜນຊັບຊ້ອນປີ|>ແຜນຊັບຊ້ອນປີ|>ແຜນຊັບຊ້ອນປີ|>ໝາຍເຫດ    "
    ...
    Private Sub loaddata()
        ...
        With RSC
            Dim aa As String
            aa = "SELECT EN_bill, EN_DT, Sections_id, Sections, Department_id, Department, year_to, year1, year2, year3 " & _
               " from AP_EN  where 1=1      " & Sql & " " & shr_section & " " & shr_Department & "   ORDER BY EN_Bill "
            Call LoadRs(aa, RSC)
            ...
            If .RecordCount <> 0 Then
                ...
                While Not .EOF()
                    Fg1.AddItem(.AbsolutePosition & _
                                 Chr(9) & Trim((RSC.Fields("EN_bill").Value).ToString) & _
                                Chr(9) & Trim((RSC.Fields("Sections").Value).ToString) & _
                                  Chr(9) & Trim((RSC.Fields("Department").Value).ToString) & _
                                Chr(9) & Format(CDate(RSC.Fields("EN_DT").Value), "dd/MM/yyyy") & _
                                  Chr(9) & Trim((RSC.Fields("year_to").Value).ToString))
    ...
    Private Sub loaddata_list()
        ...
        With RSC
            Dim aa As String
            aa = "SELECT  * from AP_EN_Item  where EN_Bill=N'" & SaleID & "'   "
            Call LoadRs(aa, RSC)

            If .RecordCount <> 0 Then

                While Not .EOF()
                    FG2.AddItem((.Fields("EN_ID").Value.ToString) & _
            Chr(9) & (.Fields("EN_nm").Value.ToString) & _
             Chr(9) & (.Fields("Toltal_all").Value.ToString) & _
              Chr(9) & (.Fields("Toltal_now").Value.ToString) & _
              Chr(9) & (.Fields("borlihan_now").Value.ToString) & _
              Chr(9) & (.Fields("visakan_now").Value.ToString) & _
              Chr(9) & (.Fields("Toltal").Value.ToString) & _
                  Chr(9) & (.Fields("borlihan").Value.ToString) & _
                      Chr(9) & (.Fields("visakan").Value.ToString) & _
                          Chr(9) & (.Fields("EN_year1").Value.ToString) & _
                           Chr(9) & (.Fields("EN_year2").Value.ToString) & _
                            Chr(9) & (.Fields("EN_year3").Value.ToString) & _
           Chr(9) & (.Fields("Remark").Value.ToString))
    ...
    Private Sub Button9_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnAddNew.Click
        ...
        MDEdit = False
        Frm_EN_Table.MdiParent = FrmAPInvioce
        Frm_EN_Table.WindowState = FormWindowState.Maximized
        Frm_EN_Table.ShowIcon = False
        Frm_EN_Table.Show()
    End Sub
