Public Class Frm_son_List
    ...
    Private Sub Button1_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Me.Close()
    End Sub
    Private Sub Frm_son_List_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load

        Fg1.FormatString = ">ລ/ດ |<ເລກທີຂໍ້ຕົກລົງ  |^ວັນທີຕົກລົງ  |<ລະຫັດພະນັກງານ|<ຊື່ ແລະ ນາມສະກຸນ |<ຊື່ (ພາສາອັງກິດ)      |<ເບີໂທລະສັບ     |<ບ່ອນປະຈຳການ   |<ພະແນກ        |>ຈຳນວນລູກ|>ລວມເງີນອຸດໜູນ|<ສະຖານະການເບີກຈ່າຍ |<ລະອຽດຂອງລູກທີ່ໄດ້ຮັບເງີນອຸດໜູນ|<ບ້ານຢູ່ປະຈຸບັນ |<ເມືອງ             |<ແຂວງ            "
    ...
    Private Sub loaddata()
        ...
        With RSC
            Dim aa As String
            aa = "SELECT     AP_Son_Money.*,AP_CV.DT_strt_work, AP_CV.start_work,AP_CV.DT_Work_now, AP_CV.txtV_C,  AP_Village.Vl_nm, AP_District.Dt_nm, AP_Province.PV_nm " & _
            " FROM         AP_Son_Money INNER JOIN " & _
          "     AP_CV ON AP_Son_Money.E_ID = AP_CV.E_ID INNER JOIN " & _
                                ...
                                Chr(9) & Trim((RSC.Fields("Name_L").Value).ToString) & _
                                 ...
                                 Chr(9) & Trim((RSC.Fields("Sections").Value).ToString) & _
                                  Chr(9) & Trim((RSC.Fields("Department").Value).ToString) & _
                                    Chr(9) & Trim((RSC.Fields("txtson").Value).ToString) & " ຄົນ" & _
                                   Chr(9) & Format(CDbl(.Fields("txtson_Money").Value), "##,##0.00") & " ກີບ" & _
                                     Chr(9) & Trim((RSC.Fields("status_nm").Value).ToString) & _
                                      Chr(9) & Trim((RSC.Fields("txtabount_son").Value).ToString) & _
    ...
    Private Sub Button9_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnAddNew.Click
        ...
        EditActive = False
        'Frm_Son.MdiParent = FrmAPInvioce
        'Frm_Son.WindowState = FormWindowState.Maximized
        'Frm_Son.ShowIcon = False
        Frm_Son.Show()
    End Sub
    ...
    Private Sub Button7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDel.Click
        ...
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການ ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            ...
            Conn.Execute(" UPDATE AP_CV SET txtToltal_All=Total_remaining-Tax+khongsep+txtMom_mony " & _
            " WHERE E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'")
   ...
   " WHERE E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'")
            Conn.Execute(" UPDATE AP_Salary SET txtToltal_All=Total_remaining-Tax+khongsep+txtMom_mony " & _
            " WHERE E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'")
            ...
            " txtson=0" & _
             " WHERE E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'")
            Conn.Execute("Delete From AP_Son_Money Where  bill_no='" & Trim(Fg1.get_TextMatrix(Fg1.Row, 1) & "' and  E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'"))
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        With RSC
            Dim sa As String = "SELECT     AP_Son_Money.*,AP_CV.DT_strt_work, AP_CV.start_work,AP_CV.DT_Work_now, AP_CV.txtV_C,  AP_Village.Vl_nm, AP_District.Dt_nm, AP_Province.PV_nm " & _
            " FROM         AP_Son_Money INNER JOIN " & _
          "     AP_CV ON AP_Son_Money.E_ID = AP_CV.E_ID INNER JOIN " & _
           "    AP_Village ON AP_CV.Add_Vill_ID = AP_Village.Vl_ID INNER JOIN " & _
           "    AP_District ON AP_Village.Dt_id = AP_District.Dt_id INNER JOIN " & _
          "     AP_Province ON AP_District.PV_id = AP_Province.PV_ID   where 1=1  " & shr_status & "" & Sql & " " & shr_section & " " & shr_Department & "  " & shr_job_phuk & "   ORDER BY  AP_CV.E_ID "
            ...
            Dim Rpt As New Report_Son_list
    ...
    Private Sub txtTdate_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtTdate.ValueChanged
        Sql = " AND AP_Son_Money.DT_in between '" & Format(txtFdate.Value, "MM/dd/yyyy") & "'AND '" & Format(txtTdate.Value, "MM/dd/yyyy") & "'"
        Call loaddata()
    End Sub
    ...
    Private Sub Button6_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button6.Click
        ...
        Dim aa As String
        Dim rs As New ADODB.Recordset
        aa = "SELECT    * from AP_Son_Money  where bill_no='" & Trim(Fg1.get_TextMatrix(Fg1.Row, 1)) & "' and  E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "' "
        Call LoadRs(aa, rs)
        With rs
            If .RecordCount > 0 Then
...
" WHERE E_ID= '" & Trim(Fg1.get_TextMatrix(Fg1.Row, 3)) & "'")
    ...
    Private Sub chk_status_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chk_status.CheckedChanged
        If chk_status.Checked = True Then
            ...
            shr_status = " AND AP_Son_Money.status_nm = N'" & cmb_status.Text & "' "
        Else
            cmb_status.Text = ""
            shr_status = ""
