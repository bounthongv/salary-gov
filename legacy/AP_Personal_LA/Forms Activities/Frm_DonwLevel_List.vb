Public Class Frm_DonwLevel_List
    ...
    Private Sub Button1_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Me.Close()
    End Sub
    Private Sub Frm_DonwLevel_List_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        FG2.FormatString = ">ລ/ດ     |<ລະຫັດພະນັກງານ|<ເລກທີ       |<ເລກທີຂໍ້ຕົກລົງ        |<ເລກທີບັນຊີທຽບຊັ້ນ-ຂັ້ນ |^ວັນທີ      |<ບ່ອນປະຈຳການ   |<ພະແນກ      |<ປະເພດການເລື່ອນຊັ້ນ-ຂັ້ນ  |<ຊື່ ແລະ ນາມສະກຸນ|>ຊັ້ນ/ຂັນເກົ່າ  |>ເງີນເດືອນຕາມຊັ້ນ/ຂັ້ນເກົ່າ|>ຊັ້ນ/ຂັນໃໝ່|>ເງີນເດືອນຕາມ ຊັ້ນ/ຂັ້ນໃໝ່|>ເງີນອຸດໜູນຕຳແໜ່ງ|>ເງີນອຸດໜູນປີການ|>ລວມເງີນ         |>ເງີນປະກັນສັງຄົມ(8%)|>ລວມເງີນຍັງເຫຼືອ|>ອາກອນເງີນເດືອນ|>ເງີນອຸດໜູນຄ່າຄອງຊີບ|>ຈ/ນລູກ|>ເງີນອຸດໜູນ ລູກ|>ຈ/ນເມຍ|>ງີນອຸດໜູນ ເມຍ|>ລວມເງີນໄດຕົວຈີງ"
    ...
    Private Sub loaddata()
        ...
        With RSC
            ...
            aa = "SELECT    AP_CV.*, AP_Village.Vl_nm, AP_District.Dt_id, AP_District.Dt_nm, AP_Province.PV_ID, AP_Province.PV_nm, " & _
                   ...
                   "   AP_Province AS AP_Province_1 ON AP_District_1.PV_id = AP_Province_1.PV_ID  where 1=1 and AP_CV.Status_donw=1   " & Sql & " " & shr_section & " " & shr_Department & "  " & shr_job_phuk & "   ORDER BY  AP_CV.E_ID "
            Call LoadRs(aa, RSC)
    ...
    Private Sub loaddata_list()
        ...
        With RSC
            Dim aa As String
            aa = "SELECT  * from AP_Donw_Personal  where E_ID=N'" & SaleID & "'    ORDER BY Bill_no "
            Call LoadRs(aa, RSC)
                    ...
                    FG2.AddItem(.AbsolutePosition & _
                               ...
                               Chr(9) & Trim((RSC.Fields("txtkhor_tok_long").Value).ToString) & _
                                Chr(9) & Trim((RSC.Fields("txtAccount_clss_level").Value).ToString) & _
                                      Chr(9) & Format(CDate(RSC.Fields("DT_dn").Value), "dd/MM/yyyy") & _
                               Chr(9) & Trim((RSC.Fields("Sections").Value).ToString) & _
                             Chr(9) & Trim((RSC.Fields("Department").Value).ToString) & _
                               Chr(9) & Trim((RSC.Fields("type_dn").Value).ToString) & _
                                 Chr(9) & Trim((RSC.Fields("Name_L").Value).ToString) & _
    ...
    Private Sub Button7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDel.Click
        ...
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການ ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From AP_Donw_Personal Where  E_ID='" & Trim(Fg1.get_TextMatrix(Fg1.Row, 1) & "'"))
        End If
        ...
        With RSC
            Dim sa As String = "SELECT     AP_CV.type_in_id, AP_CV.type_in_nm, AP_CV.DT_strt_work, AP_CV.start_work_ID, AP_CV.start_work, AP_CV.DT_Work_now, AP_CV.cmbclass, " & _
                  "    AP_CV.cmblevel, AP_CV.job_lut_ID, AP_CV.job_lut, AP_CV.Add_Vill_ID, AP_Village.Vl_nm, AP_District.Dt_id, AP_District.Dt_nm, " & _
                   "   AP_Province.PV_ID, AP_Province.PV_nm, AP_Donw_Personal.* " & _
                   "       FROM         AP_CV INNER JOIN " & _
                   "   AP_Village ON AP_CV.Add_Vill_ID = AP_Village.Vl_ID INNER JOIN" & _
                     " AP_District ON AP_Village.Dt_id = AP_District.Dt_id INNER JOIN " & _
                 "     AP_Province ON AP_District.PV_id = AP_Province.PV_ID INNER JOIN " & _
                  "    AP_Donw_Personal ON AP_CV.E_ID = AP_Donw_Personal.E_ID  WHERE 1=1   " & Sql & " " & shr_section & " " & shr_Department & "  " & shr_job_phuk & "   ORDER BY  AP_CV.E_ID "
            Call LoadRs(sa, RSC)
            If .RecordCount = 0 Then MsgBox("Data empty", vbInformation, "Check") : Exit Sub
            Dim Frm As New FrmPreview
            Dim Rpt As New Report_Salary_donw_list
