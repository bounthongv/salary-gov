Public Class Frm_CV_List
    ...
    Private Sub Frm_CV_List_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Fg1.FormatString = "ລ/ດ |<ລະຫັດພະນັກງານ |<ຊື່ ແລະ ນາມສະກຸນ                     |<ຊື່ (ພາສາອັງກິດ)                      |<ເບີໂທລະສັບ  |<ບ່ອນປະຈຳການ     |<ພະແນກ           |<ໜ້າທີ່ຮັບຜິດຊອບ   |^ວັນທີເລີມເຮັດການ|<ຊັ້ນ/ຂັນ|^ວັນທີ່ເລື່ອນຊັ້ນຂັ້ນ|<ຊັ້ນ/ຂັນ|<ປະເພດພະນັກງານ         "
  ...
  "    FROM         AP_CV INNER JOIN " & _
      "   AP_Village ON AP_CV.BVill_ID = AP_Village.Vl_ID INNER JOIN " & _
               "       AP_District ON AP_Village.Dt_id = AP_District.Dt_id INNER JOIN " & _
                ...
                "   where 1=1 " & Sql & " " & shr_section & " " & shr_Department & " " & shr_job_phuk & "" & shr_type_in & " " & shr_job_lut & "   ORDER BY   AP_CV.order_no   "
            Call LoadRs(aa, RSC)
    ...
    Private Sub Button7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDel.Click
        ...
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການ : " & Trim(Fg1.get_TextMatrix(Fg1.Row, 1)) & "  ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From AP_CV Where  e_id='" & Trim(Fg1.get_TextMatrix(Fg1.Row, 1) & "'"))
            ...
            "    FROM         AP_CV INNER JOIN " & _
                "   AP_Village ON AP_CV.BVill_ID = AP_Village.Vl_ID INNER JOIN " & _
                         "       AP_District ON AP_Village.Dt_id = AP_District.Dt_id INNER JOIN " & _
                             ...
                             "   Type_In ON AP_CV.type_in_id = Type_In.In_ID LEFT OUTER JOIN " & _
                            "    Department ON AP_CV.Department_id = Department.DP_ID LEFT OUTER JOIN " & _
                             "   AP_Sections ON AP_CV.Sections_id = AP_Sections.Sec_id   where 1=1   " & Sql & " " & shr_section & " " & shr_Department & " " & shr_job_phuk & "" & shr_type_in & "  ORDER BY  AP_CV.E_id  "
            Call LoadRs(aa, RSC)
 ...
 "    FROM         AP_CV INNER JOIN " & _
     "   AP_Village ON AP_CV.BVill_ID = AP_Village.Vl_ID INNER JOIN " & _
              "       AP_District ON AP_Village.Dt_id = AP_District.Dt_id INNER JOIN " & _
                 ...
                 "    Department ON AP_CV.Department_id = Department.DP_ID LEFT OUTER JOIN " & _
                  "   AP_Sections ON AP_CV.Sections_id = AP_Sections.Sec_id    " & _
               "   where 1=1 and  AP_CV.e_id='" & SaleID & "' "
        Call LoadRs(aa, rs)
        With rs
            If .RecordCount > 0 Then
 ...
 "    FROM         AP_CV INNER JOIN " & _
    ...
    Private Sub Button16_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button16.Click
        Conn.Execute(" update ap_cv set order_no='" & txtorder_no.Text & "' where AP_CV.e_id='" & SaleID & "'  ")
