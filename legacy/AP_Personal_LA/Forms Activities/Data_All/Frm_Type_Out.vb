Public Class Frm_Type_Out
    ...
    Private Sub AutoNumber()
        ...
        Call LoadRs("SELECT top 1 OUT_ID from Type_OUT    Order by OUT_ID DESC", VIOT)
        If VIOT.RecordCount <> 0 Then
            VIOTNEW = Format(Val(Mid(VIOT.Fields("OUT_ID").Value, 1, 2)) + 1, "0")
        Else
            VIOTNEW = "1"
    ...
    End Sub
    Public Sub save()
        Call LoadRs("SELECT * FROM Type_OUT WHERE OUT_ID = '" & Txt_ID.Text & "'  ", rs)
        With rs
            If .RecordCount = 0 Then
                Conn.Execute("INSERT INTO Type_OUT (OUT_ID,OUT_Nm) " & _
                   " VALUES(" & CDbl(Txt_ID.Text) & "," & _
                                 " N'" & Txt_name.Text & "')")
            Else
                Conn.Execute("UPDATE Type_OUT SET " & _
                   " OUT_ID=" & CDbl(Txt_ID.Text) & ", " & _
                        " OUT_Nm=N'" & Txt_name.Text & "' " & _
                   " WHERE OUT_ID= '" & (Txt_ID.Text) & "'  ")
            End If
        End With
    ...
    Public Sub LoadData()
        fg.Rows = 1
        With rs
            Call LoadRs("SELECT   * from Type_OUT order by  OUT_ID", rs)
            If .RecordCount > 0 Then
                While Not .EOF()
                    fg.AddItem(.AbsolutePosition & _
                    Chr(9) & (.Fields("OUT_ID").Value.ToString) & _
                    Chr(9) & (.Fields("OUT_Nm").Value.ToString))
                    .MoveNext()
                End While
            End If
        End With
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການນີ້: " & Trim(fg.get_TextMatrix(fg.Row, 1)) & "  ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From Type_OUT Where  OUT_ID=N'" & Trim(fg.get_TextMatrix(fg.Row, 1)) & "' ")
            Call LoadData()
        End If
