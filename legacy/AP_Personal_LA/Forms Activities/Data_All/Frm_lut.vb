Public Class Frm_lut
    Dim rs As New ADODB.Recordset

    Private Sub Frm_lut_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub AutoNumber()
        ...
        Call LoadRs("SELECT top 1 lut_ID from lut     Order by lut_ID DESC", VIOT)
        If VIOT.RecordCount <> 0 Then
            VIOTNEW = Format(Val(Mid(VIOT.Fields("lut_ID").Value, 1, 3)) + 1, "000")
        Else
            VIOTNEW = "001"
    ...
    End Sub
    Public Sub save()
        Call LoadRs("SELECT * FROM lut WHERE lut_ID = '" & Txt_ID.Text & "'  ", rs)
        With rs
            If .RecordCount = 0 Then
                Conn.Execute("INSERT INTO lut (lut_ID,lut_nm) " & _
                   " VALUES('" & (Txt_ID.Text) & "'," & _
                                 " N'" & Txt_name.Text & "')")
            Else
                Conn.Execute("UPDATE lut SET " & _
                        " lut_nm=N'" & Txt_name.Text & "' " & _
                   " WHERE lut_ID= '" & (Txt_ID.Text) & "'  ")
            End If
        End With
    ...
    Public Sub LoadData()
        fg.Rows = 1
        With rs
            Call LoadRs("SELECT   * from lut order by  lut_ID", rs)
            If .RecordCount > 0 Then
                While Not .EOF()
                    fg.AddItem(.AbsolutePosition & _
                    Chr(9) & (.Fields("lut_ID").Value.ToString) & _
                    Chr(9) & (.Fields("lut_nm").Value.ToString))
                    .MoveNext()
                End While
            End If
        End With
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການນີ້: " & Trim(fg.get_TextMatrix(fg.Row, 1)) & "  ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From lut Where  lut_ID=N'" & Trim(fg.get_TextMatrix(fg.Row, 1)) & "' ")
            Call LoadData()
        End If
