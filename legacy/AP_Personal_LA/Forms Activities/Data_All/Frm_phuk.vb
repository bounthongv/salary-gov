Public Class Frm_phuk
    Dim rs As New ADODB.Recordset

    Private Sub Frm_phuk_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub AutoNumber()
        ...
        Call LoadRs("SELECT top 1 phuk_id from phuk    Order by phuk_id DESC", VIOT)
        If VIOT.RecordCount <> 0 Then
            VIOTNEW = Format(Val(Mid(VIOT.Fields("phuk_id").Value, 1, 3)) + 1, "000")
        Else
            VIOTNEW = "001"
    ...
    End Sub
    Public Sub save()
        Call LoadRs("SELECT * FROM phuk WHERE phuk_id = '" & Txt_ID.Text & "'  ", rs)
        With rs
            If .RecordCount = 0 Then
                Conn.Execute("INSERT INTO phuk (phuk_id,phuk_nm,phuk_nm2) " & _
                   " VALUES('" & (Txt_ID.Text) & "'," & _
                  " N'" & (Txt_name.Text) & "'," & _
                                 " N'" & Txt_name2.Text & "')")
            Else
                Conn.Execute("UPDATE phuk SET " & _
                       " phuk_nm=N'" & Txt_name.Text & "', " & _
                           " phuk_nm2=N'" & Txt_name2.Text & "' " & _
                   " WHERE phuk_id= '" & (Txt_ID.Text) & "'  ")
            End If
        End With
    ...
    Public Sub LoadData()
        fg.Rows = 1
        With rs
            Call LoadRs("SELECT   * from phuk order by  phuk_id", rs)
            If .RecordCount > 0 Then
                While Not .EOF()
                    fg.AddItem(.AbsolutePosition & _
                    Chr(9) & (.Fields("phuk_id").Value.ToString) & _
                      Chr(9) & (.Fields("phuk_nm").Value.ToString) & _
                    Chr(9) & (.Fields("phuk_nm2").Value.ToString))
                    .MoveNext()
                End While
            End If
        End With
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການນີ້: " & Trim(fg.get_TextMatrix(fg.Row, 1)) & "  ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From phuk Where  phuk_ID=N'" & Trim(fg.get_TextMatrix(fg.Row, 1)) & "' ")
            Call LoadData()
        End If
