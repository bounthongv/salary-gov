Public Class Frm_Level
    ...
    End Sub
    Public Sub save()
        Call LoadRs("SELECT * FROM Level WHERE LV_ID = '" & Txt_ID.Text & "'  ", rs)
        With rs
            ...
            Else
                Conn.Execute("UPDATE Level SET " & _
                   " LV_ID=" & CDbl(Txt_ID.Text) & ", " & _
                        " LV_Nm=N'" & Txt_name.Text & "' " & _
                   " WHERE LV_ID= '" & (Txt_ID.Text) & "'  ")
            End If
        End With
    ...
    Public Sub LoadData()
        fg.Rows = 1
        With rs
            Call LoadRs("SELECT   * from Level order by  LV_ID", rs)
            If .RecordCount > 0 Then
                While Not .EOF()
                    fg.AddItem(.AbsolutePosition & _
                    Chr(9) & (.Fields("LV_ID").Value.ToString) & _
                    Chr(9) & (.Fields("LV_Nm").Value.ToString))
                    .MoveNext()
                End While
            End If
        End With
