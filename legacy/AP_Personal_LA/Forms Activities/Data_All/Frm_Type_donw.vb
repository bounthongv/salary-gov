Public Class Frm_Type_donw
    ...
    Private Sub AutoNumber()
        ...
        Call LoadRs("SELECT top 1 dn_id from Type_Donw    Order by dn_id DESC", VIOT)
        If VIOT.RecordCount <> 0 Then
            VIOTNEW = Format(Val(Mid(VIOT.Fields("dn_id").Value, 1, 3)) + 1, "000")
        Else
            VIOTNEW = "001"
    ...
    End Sub
    Public Sub save()
        Call LoadRs("SELECT * FROM Type_Donw WHERE dn_id = '" & Txt_ID.Text & "'  ", rs)
        With rs
            If .RecordCount = 0 Then
                Conn.Execute("INSERT INTO Type_Donw (dn_id,dn_nm) " & _
                   " VALUES('" & (Txt_ID.Text) & "'," & _
                                 " N'" & Txt_name.Text & "')")
            Else
                Conn.Execute("UPDATE Type_Donw SET " & _
                       " dn_nm=N'" & Txt_name.Text & "' " & _
    ...
    Public Sub LoadData()
        fg.Rows = 1
        With rs
            Call LoadRs("SELECT   * from Type_Donw order by dn_id ", rs)
            If .RecordCount > 0 Then
                While Not .EOF()
                    fg.AddItem(.AbsolutePosition & _
                    Chr(9) & (.Fields("dn_id").Value.ToString) & _
                    Chr(9) & (.Fields("dn_nm").Value.ToString))
                    .MoveNext()
                End While
            End If
        End With
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການນີ້: " & Trim(fg.get_TextMatrix(fg.Row, 1)) & "  ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From Type_donw Where  dn_ID=N'" & Trim(fg.get_TextMatrix(fg.Row, 1)) & "' ")
            Call LoadData()
        End If
