Public Class Frm_Study
    ...
    Private Sub AutoNumber()
        ...
        Call LoadRs("SELECT top 1 Field_ID from Study_Field    Order by Field_ID DESC", VIOT)
        If VIOT.RecordCount <> 0 Then
            VIOTNEW = Format(Val(Mid(VIOT.Fields("Field_ID").Value, 1, 3)) + 1, "000")
        Else
            VIOTNEW = "001"
    ...
    End Sub
    Public Sub save()
        Call LoadRs("SELECT * FROM Study_Field WHERE Field_ID = '" & Txt_ID.Text & "'  ", rs)
        With rs
            If .RecordCount = 0 Then
                Conn.Execute("INSERT INTO Study_Field (Field_ID,Field_Name) " & _
                   " VALUES('" & (Txt_ID.Text) & "'," & _
                                 " N'" & Txt_name.Text & "')")
            Else
                Conn.Execute("UPDATE Study_Field SET " & _
                        " Field_Name=N'" & Txt_name.Text & "' " & _
                   " WHERE Field_ID= '" & (Txt_ID.Text) & "'  ")
            End If
        End With
    ...
    Public Sub LoadData()
        fg.Rows = 1
        With rs
            Call LoadRs("SELECT   * from Study_Field    order by  Field_ID", rs)
            If .RecordCount > 0 Then
                While Not .EOF()
                    fg.AddItem(.AbsolutePosition & _
                    Chr(9) & (.Fields("Field_ID").Value.ToString) & _
                    Chr(9) & (.Fields("Field_Name").Value.ToString))
                    .MoveNext()
                End While
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການນີ້: " & Trim(fg.get_TextMatrix(fg.Row, 1)) & "  ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From Study_Field Where  Field_ID=N'" & Trim(fg.get_TextMatrix(fg.Row, 1)) & "' ")
            Call LoadData()
        End If
