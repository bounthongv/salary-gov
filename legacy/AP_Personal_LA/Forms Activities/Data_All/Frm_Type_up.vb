Public Class Frm_Type_up
    Dim rs As New ADODB.Recordset

    Private Sub Frm_Type_up_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub AutoNumber()
        ...
        Call LoadRs("SELECT top 1 up_id from Type_up    Order by up_id DESC", VIOT)
        If VIOT.RecordCount <> 0 Then
            VIOTNEW = Format(Val(Mid(VIOT.Fields("up_id").Value, 1, 3)) + 1, "000")
        Else
            VIOTNEW = "001"
    ...
    End Sub
    Public Sub save()
        Call LoadRs("SELECT * FROM Type_up WHERE up_id = '" & Txt_ID.Text & "'  ", rs)
        With rs
            If .RecordCount = 0 Then
                Conn.Execute("INSERT INTO Type_up (up_id,up_nm) " & _
                   " VALUES('" & (Txt_ID.Text) & "'," & _
                                 " N'" & Txt_name.Text & "')")
            Else
                Conn.Execute("UPDATE Type_up SET " & _
                       " up_nm=N'" & Txt_name.Text & "' " & _
                   " WHERE up_id= '" & (Txt_ID.Text) & "'  ")
            End If
        End With
    ...
    Public Sub LoadData()
        fg.Rows = 1
        With rs
            Call LoadRs("SELECT   * from Type_up order by  up_id", rs)
            If .RecordCount > 0 Then
                While Not .EOF()
                    fg.AddItem(.AbsolutePosition & _
                    Chr(9) & (.Fields("up_id").Value.ToString) & _
                    Chr(9) & (.Fields("up_nm").Value.ToString))
                    .MoveNext()
                End While
            End If
        End With
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການນີ້: " & Trim(fg.get_TextMatrix(fg.Row, 1)) & "  ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From Type_up Where  up_ID=N'" & Trim(fg.get_TextMatrix(fg.Row, 1)) & "' ")
            Call LoadData()
        End If
