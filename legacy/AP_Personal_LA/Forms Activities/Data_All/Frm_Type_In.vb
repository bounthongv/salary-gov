Public Class Frm_Type_In
    ...
    Private Sub AutoNumber()
        ...
        Call LoadRs("SELECT top 1 In_ID from Type_In    Order by In_ID DESC", VIOT)
        If VIOT.RecordCount <> 0 Then
            VIOTNEW = Format(Val(Mid(VIOT.Fields("In_ID").Value, 1, 2)) + 1, "0")
        Else
            VIOTNEW = "1"
    ...
    End Sub
    Public Sub save()
        Call LoadRs("SELECT * FROM Type_In WHERE In_ID = '" & Txt_ID.Text & "'  ", rs)
        With rs
            If .RecordCount = 0 Then
                Conn.Execute("INSERT INTO Type_In (In_ID,In_nm) " & _
                   " VALUES(" & CDbl(Txt_ID.Text) & "," & _
                                 " N'" & Txt_name.Text & "')")
            Else
                Conn.Execute("UPDATE Type_In SET " & _
                   " In_ID=" & CDbl(Txt_ID.Text) & ", " & _
                        " In_nm=N'" & Txt_name.Text & "' " & _
                   " WHERE In_ID= '" & (Txt_ID.Text) & "'  ")
            End If
        End With
    ...
    Public Sub LoadData()
        fg.Rows = 1
        With rs
            Call LoadRs("SELECT   * from Type_In order by  In_ID", rs)
            If .RecordCount > 0 Then
                While Not .EOF()
                    fg.AddItem(.AbsolutePosition & _
                    Chr(9) & (.Fields("In_ID").Value.ToString) & _
                    Chr(9) & (.Fields("In_nm").Value.ToString))
                    .MoveNext()
                End While
            End If
        End With
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການນີ້: " & Trim(fg.get_TextMatrix(fg.Row, 1)) & "  ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From Type_In Where  In_ID=N'" & Trim(fg.get_TextMatrix(fg.Row, 1)) & "' ")
            Call LoadData()
        End If
