Public Class Frm_Mom

    Private Sub Frm_Mom_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub Editdata()
        ...
        aa = "SELECT    * from AP_mom_Money  where E_ID=N'" & E_ID & "' and bill_no=N'" & Bill_ID & "' "
        Call LoadRs(aa, rs)
        With rs
            If .RecordCount > 0 Then
                ...
                txtabount_mom.Text = Trim(.Fields("txtabount_mom").Value.ToString)
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Dim rs As New ADODB.Recordset
        Call LoadRs("SELECT * FROM  AP_mom_Money WHERE   E_ID = N'" & CustID & "' and  status =0 ", rs)
        If rs.RecordCount > 0 Then
            ...
            txtmom.Text = (rs.Fields("txtmom").Value.ToString)
            txtmom_Money.Text = Format(CDbl(rs.Fields("txtmom_Money").Value), "##,##0.00")
            txtabount_mom.Text = (rs.Fields("txtabount_mom").Value.ToString)
        Else
            AutoNumber()
            DT_in.Value = Today
            txtmom.Text = 0
    ...
    Private Sub LoadData_CV()
        ...
        With rs
            ...
            If .RecordCount <> 0 Then
                ...
                'txtmom.Text = (.Fields("txtson").Value.ToString)
                'txtmom_Money.Text = Format(CDbl(.Fields("txtson_Money").Value), "##,##0.00")
    ...
    Private Sub AutoNumber()
        ...
        Call LoadRs("SELECT top 1 Bill_no from AP_mom_Money    Order by Bill_no DESC", VIOT)
    ...
    Private Sub Save()
        ...
        Dim rs As New ADODB.Recordset
        With rs
            Call LoadRs("SELECT * FROM  AP_mom_Money WHERE bill_no = N'" & txt_no.Text & "' and  E_ID = N'" & txtid.Text & "'", rs)
            If .RecordCount = 0 Then
                Dim aa As String
                aa = "INSERT INTO   AP_mom_Money ( bill_no, DT_in, E_ID, Sections_id, Sections, Department_id, Department, Name_L, Name_E, Phone, " & _
                " txtmom, txtmom_Money,txtabount_mom, remark, lst_updt, lst_usr, Pc_nm) " & _
               " VALUES(N'" & (txt_no.Text) & "'," & _
                 ...
                 " N'" & (TxtPersonNmE.Text) & "'," & _
                      " N'" & (TxtTel.Text) & "'," & _
                          " " & CDbl(txtmom.Text) & "," & _
                           " " & CDbl(txtmom_Money.Text) & "," & _
                               " N'" & (txtabount_mom.Text) & "'," & _
                               " N'" & (txtremark.Text) & "'," & _
                                        " Getdate()," & _
                               " N'" & MUserName & "'," & _
                            " '" & MDServerName & "')"
                Conn.Execute(aa)
            Else
                Conn.Execute("delete from   AP_mom_Money WHERE    E_ID= N'" & (txtid.Text) & "' and  bill_no= N'" & (txt_no.Text) & "'")
                Dim aa As String
                aa = "INSERT INTO   AP_mom_Money ( bill_no, DT_in, E_ID, Sections_id, Sections, Department_id, Department, Name_L, Name_E, Phone, " & _
                            " txtmom, txtmom_Money,txtabount_mom, remark, lst_updt, lst_usr, Pc_nm) " & _
                           " VALUES(N'" & (txt_no.Text) & "'," & _
        ...
        Dim rsc As New ADODB.Recordset
        Call LoadRs("SELECT * FROM  AP_mom_Money WHERE bill_no = N'" & txt_no.Text & "' and  E_ID = N'" & txtid.Text & "' and  status =0 ", rsc)
        If rsc.RecordCount > 0 Then

            Conn.Execute(" UPDATE AP_CV SET txtToltal_All=0 " & _
             " WHERE E_ID= '" & (txtid.Text) & "'")

            Conn.Execute(" UPDATE AP_CV SET txtToltal_All=Total_remaining-Tax+khongsep+" & CDbl(txtmom_Money.Text) & "+txtson_money " & _
            " WHERE E_ID= '" & (txtid.Text) & "'")

            Conn.Execute(" UPDATE AP_CV SET " & _
                  " Status_mom=1," & _
                          " txtmom_Mony=" & CDbl(txtmom_Money.Text) & "," & _
          ...
          " WHERE E_ID= '" & (txtid.Text) & "'")
            ...
            Conn.Execute(" UPDATE AP_Salary SET txtToltal_All=Total_remaining-Tax+khongsep+" & CDbl(txtmom_Money.Text) & "+txtson_money " & _
            " WHERE E_ID= '" & (txtid.Text) & "'")
      ...
      " status_nm=N'ຍັງອະນຸມັດຈ່າຍຢູ່' " & _
       " WHERE bill_no= N'" & txt_no.Text & "' and E_ID= N'" & txtid.Text & "'")
        End If
    End Sub

    Private Sub txtson_Money_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtmom_Money.KeyDown
        Select Case e.KeyCode
            Case Keys.Enter
                txtmom_Money.Text = Format(CDbl(txtmom_Money.Text), "##,##0.00")
        End Select
    End Sub

    Private Sub txtson_Money_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtmom_Money.TextChanged
