Public Class Frm_Position_work
    ...
    Private Sub Button1_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Me.Close()
    End Sub
    Private Sub Frm_Position_work_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Fg1.FormatString = "ລ/ດ |<ລະຫັດ  |^ວັນທີ        |<ລະຫັດພະນັກງານ |<ຊື່ ແລະ ນາມສະກຸນ   |<ບ່ອນປະຈຳການ   |<ພະແນກ    |<ໜ້າທີ່ຮັບຜິດຊອບລົງເລິກ ວຽກງານ|<ຕຳແໜ່ງພັກ   |<ຕຳແໜ່ງລັດ ບໍລິຫານ|<ຕຳແໜ່ງລັດ ວິຊາການ "
    ...
    Private Sub AutoNumber()
        ...
        Call LoadRs("SELECT top 1 Bill_no from AP_Position    Order by bill_no DESC", VIOT)
    ...
    Private Sub loaddata()
        ...
        With RSC
            Dim aa As String
            aa = "SELECT    AP_Position.*,AP_CV.Sections,AP_CV.Department  " & _
                   "   FROM         AP_Position INNER JOIN " & _
                   "   AP_CV ON AP_Position.E_ID = AP_CV.E_ID   where 1=1   " & Sql & " " & _
                   " " & shr_section & " " & shr_Department & " " & shr_type_in & "  " & shr_job_phuk & " " & shr_job_lut & " " & shr_vsakan & "" & shr_duties & "    ORDER BY  AP_CV.E_ID "
            Call LoadRs(aa, RSC)
            ...
            If .RecordCount <> 0 Then
                ...
                While Not .EOF()
                    Fg1.AddItem(.AbsolutePosition & _
                         ...
                         Chr(9) & Format(CDate(RSC.Fields("DT_Pos").Value), "dd/MM/yyyy") & _
                                ...
                                Chr(9) & Trim((RSC.Fields("duties").Value).ToString) & _
                                     Chr(9) & Trim((RSC.Fields("job_phuk").Value).ToString) & _
                                      Chr(9) & Trim((RSC.Fields("job_lut").Value).ToString) & _
                                  Chr(9) & Trim((RSC.Fields("job_lut_visakan").Value).ToString))
    ...
    Private Sub LoadData_Edit()
        ...
        Dim aa As String
        With rs
            aa = "SELECT  * from AP_Position  WHERE 1=1  AND E_ID=N'" & Fg1.get_TextMatrix(Fg1.Row, 3) & "' and bill_no=N'" & Fg1.get_TextMatrix(Fg1.Row, 1) & "'  "
            Call LoadRs(aa, rs)
            If .RecordCount <> 0 Then
                ...
                cmb_duties.Text = (.Fields("duties").Value.ToString)
                txtremark.Text = (.Fields("txtremark").Value.ToString)

                If .Fields("chk_Job_Phuk").Value = 1 Then
                    chk_Job_Phuk.Checked = True
                    cmb_job_phuk.Text = (.Fields("job_phuk").Value.ToString)
                Else
                    chk_Job_Phuk.Checked = False
                End If
                If .Fields("chk_job_lut").Value = 1 Then
                    chk_job_lut.Checked = True
                    cmb_job_lut.Text = (.Fields("job_lut").Value.ToString)
                Else
                ...
                If .Fields("chk_job_lut_visakan").Value = 1 Then
                    ...
                    cmb_job_lut_visakan.Text = (.Fields("job_lut_visakan").Value.ToString)
                Else
    ...
    Private Sub Button7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDel.Click
        ...
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການ : " & Trim(Fg1.get_TextMatrix(Fg1.Row, 1)) & "  ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From AP_Position Where  bill_no='" & txt_no.Text & "' and E_id='" & txtid.Text & "'")
    ...
    Private Sub txtTdate_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtTdate.ValueChanged
        If chk_date.Checked = True Then
            ...
            Sql = " AND AP_Position.DT_Pos between '" & Format(txtFdate.Value, "MM/dd/yyyy") & "'AND '" & Format(txtTdate.Value, "MM/dd/yyyy") & "'"
        Else
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        ...
        End If
        Save()
        E_ID = " and AP_Position.E_ID=N'" & txtid.Text & "'"
        loaddata_one()
    ...
    Private Sub Save()
        ...
        Dim rs As New ADODB.Recordset
        With rs
            Call LoadRs("SELECT * FROM  AP_Position WHERE  Bill_no= '" & (txt_no.Text) & "' and E_ID = '" & txtid.Text & "'", rs)
            If .RecordCount = 0 Then
                Dim aa As String
                aa = "INSERT INTO   AP_Position (  Bill_no, E_ID,Name_L, DT_Pos,duties_Id,duties, txtremark,lst_updt, lst_usr, Pc_nm) " & _
        ...
        End With
        If chk_Job_Phuk.Checked = True Then
            Conn.Execute(" UPDATE AP_Position SET " & _
                     " chk_Job_Phuk=1," & _
    ...
    Private Sub loaddata_one()
        ...
        With RSC
            ...
            aa = "SELECT    AP_Position.*,AP_CV.Sections,AP_CV.Department " & _
                   "   FROM         AP_Position INNER JOIN " & _
                   "   AP_CV ON AP_Position.E_ID = AP_CV.E_ID   where 1=1 " & E_ID & "    ORDER BY AP_Position.DT_Pos desc "
            Call LoadRs(aa, RSC)
    ...
    Private Sub chk_Job_Phuk2_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chk_Job_Phuk2.CheckedChanged
        If chk_Job_Phuk2.Checked = True Then
            ...
            shr_job_phuk = " AND AP_Position.chk_Job_Phuk =1"
    ...
    Private Sub cmb_job_phuk2_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmb_job_phuk2.SelectedIndexChanged
        ...
        If cmb_job_phuk2.SelectedIndex = 0 Then
            shr_job_phuk = " AND AP_Position.chk_Job_Phuk =1"
        Else
            shr_job_phuk = " AND AP_Position.job_phuk_ID = N'" & txt_job_phuk_id2.Text & "' "
        End If
    End Sub
    ...
    Private Sub cmb_job_lut2_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmb_job_lut2.SelectedIndexChanged
        ...
        If cmb_job_lut2.SelectedIndex = 0 Then
            shr_job_lut = " AND AP_Position.chk_job_lut =1"
        Else
            shr_job_lut = " AND AP_Position.job_lut_ID = N'" & txt_job_lut_id2.Text & "' "
        End If
    End Sub
    ...
    Private Sub chk_year_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chk_year.CheckedChanged
        If chk_date.Checked = True Then
            ...
            Sql = " and    year(AP_Position.DT_Pos) ='" & Year(DT_year.Value) & "' "
        Else
    ...
    Private Sub cmb_duties2_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmb_duties2.SelectedIndexChanged
        ...
        End If
        shr_duties = " AND AP_Position.duties_Id = N'" & txt_duties_id2.Text & "' "
