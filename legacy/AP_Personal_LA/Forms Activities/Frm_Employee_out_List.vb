Public Class Frm_Employee_out_List
    ...
    Private Sub Frm_Employee_out_List_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Fg1.FormatString = "ລ/ດ |<ລະຫັດພະນັກງານ |<ຊື່ ແລະ ນາມສະກຸນ      |<ຊື່ (ພາສາອັງກິດ)       |<ເບີໂທລະສັບ  |<ບ່ອນປະຈຳການ     |<ພະແນກ           |<ໜ້າທີ່ຮັບຜິດຊອບ   |^ວັນທີເລີມເຮັດການ|<ຊັ້ນ/ຂັນ|^ວັນທີ່ເລື່ອນຊັ້ນຂັ້ນ|<ຊັ້ນ/ຂັນ|<ປະເພດພະນັກງານ|<ລະອຽດຂອງການລາອອກ|<ໝາຍເຫດ        "
    ...
    Private Sub loaddata()
        ...
        With RSC
            Dim aa As String
            aa = "SELECT     AP_Employee_out.E_ID, AP_Employee_out.*, AP_Village.Vl_nm, AP_District.Dt_id, " & _
  ...
  "    FROM         AP_Employee_out INNER JOIN " & _
      "   AP_Village ON AP_Employee_out.BVill_ID = AP_Village.Vl_ID INNER JOIN " & _
               "       AP_District ON AP_Village.Dt_id = AP_District.Dt_id INNER JOIN " & _
                   ...
                   "   AP_Village AS AP_Village_1 ON AP_Employee_out.Add_Vill_ID = AP_Village_1.Vl_ID INNER JOIN " & _
                   ...
                   "   job ON AP_Employee_out.duties_Id = job.job_id LEFT OUTER JOIN " & _
                   "   Type_In ON AP_Employee_out.type_in_id = Type_In.In_ID LEFT OUTER JOIN " & _
                  "    Department ON AP_Employee_out.Department_id = Department.DP_ID LEFT OUTER JOIN " & _
                   "   AP_Sections ON AP_Employee_out.Sections_id = AP_Sections.Sec_id      " & _
            "   where 1=1 " & Sql & " " & shr_section & " " & shr_Department & " " & shr_job_phuk & "" & shr_type_in & " " & shr_job_lut & " order by  AP_Employee_out.E_id    "
            Call LoadRs(aa, RSC)
                                 ...
                                 Chr(9) & Trim((RSC.Fields("Name_E").Value).ToString) & _
                                  Chr(9) & Trim((RSC.Fields("Phone").Value).ToString) & _
                                      ...
                                      Chr(9) & Trim((RSC.Fields("txtV_C").Value).ToString) & _
                                        Chr(9) & Trim((RSC.Fields("In_nm").Value).ToString) & _
                                          Chr(9) & Trim((RSC.Fields("abount").Value).ToString) & _
    ...
    Private Sub Button9_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnAddNew.Click
        ...
        EditActive = False
        Frm_Employee_out.MdiParent = FrmAPInvioce
        Frm_Employee_out.WindowState = FormWindowState.Maximized
        Frm_Employee_out.ShowIcon = False
        Frm_Employee_out.Show()
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        With RSC
            Dim sa As String = "SELECT    AP_CV.*, AP_Village.Vl_nm, AP_District.Dt_id, AP_District.Dt_nm, AP_Province.PV_ID, AP_Province.PV_nm, " & _
                   ...
                   "   AP_Village ON AP_CV.BVill_ID = AP_Village.Vl_ID INNER JOIN  " & _
                    ...
                    "  AP_District AS AP_District_1 ON AP_Village_1.Dt_id = AP_District_1.Dt_id INNER JOIN " & _
                   "   AP_Province AS AP_Province_1 ON AP_District_1.PV_id = AP_Province_1.PV_ID  where 1=1   " & Sql & " " & shr_section & " " & shr_Department & " " & shr_job_phuk & "" & shr_type_in & "  ORDER BY  AP_CV.txtV_C  desc  "
            Call LoadRs(sa, RSC)
            If .RecordCount = 0 Then MsgBox("Data empty", vbInformation, "Check") : Exit Sub
            Dim Frm As New FrmPreview
            Dim Rpt As New Report_Persion_list
    ...
    Private Sub chk_section_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chk_section.CheckedChanged
        If chk_section.Checked = True Then
            ...
            shr_section = " AND AP_Employee_out.Sections_id = N'" & txtSection_ID.Text & "' "
    ...
    Private Sub chk_department_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chk_department.CheckedChanged
        If chk_department.Checked = True Then
            ...
            shr_Department = " AND AP_Employee_out.Department_id = N'" & txtdepart_ID.Text & "' "
