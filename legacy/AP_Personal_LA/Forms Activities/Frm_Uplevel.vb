Public Class Frm_Uplevel

    Private Sub Frm_Uplevel_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub Editdata()
        ...
        aa = "SELECT    * from AP_Up_Personal  where E_ID=N'" & E_ID & "' and Bill_no=N'" & Bill_no & "'"
        Call LoadRs(aa, rs)
        With rs
            If .RecordCount > 0 Then
                txt_no.Text = Trim(.Fields("Bill_no").Value.ToString)
                txtid.Text = Trim(.Fields("E_ID").Value.ToString)
                DT_up.Value = Trim(.Fields("DT_Up").Value.ToString)
                ...
                cmb_type_up.Text = Trim(.Fields("type_up").Value.ToString)
    ...
    Private Sub AutoNumber()
        ...
        Call LoadRs("SELECT top 1 Bill_no from AP_Up_Personal    Order by Bill_no DESC", VIOT)
    ...
    Private Sub Button15_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button15.Click
        Frm_Type_up.ShowDialog()
        cmb_type_up.Items.Clear()
        Call load_Cmb("select up_nm from Type_up", "up_nm", cmb_type_up)
        cmb_type_up.SelectedIndex = 0
    End Sub
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        Save()
        Conn.Execute(" UPDATE ap_cv SET Status_up=1 where E_ID='" & txtid.Text & "'")
    ...
    Private Sub Save()
        ...
        Dim rs As New ADODB.Recordset
        With rs
            Call LoadRs("SELECT * FROM  AP_Up_Personal WHERE  Bill_no= '" & (txt_no.Text) & "' and E_ID = '" & txtid.Text & "'", rs)
            If .RecordCount = 0 Then
                Dim aa As String
                aa = "INSERT INTO   AP_Up_Personal (  Bill_no, E_ID, DT_Up, Sections_id, Sections, Department_id, Department, Name_L, Name_E, Phone, Bank_no, SSO_no,percen_old, txtclass_old, txtlevel_old, " & _
       ...
       "     txtson, txtson_Money, txtmom, txtMom_mony, txtToltal_All, txtkhor_tok_long, txtAccount_clss_level, type_up_id, type_up,percen, remark, lst_updt, lst_usr, Pc_nm) " & _
               " VALUES(N'" & (txt_no.Text) & "'," & _
                ...
                Conn.Execute(aa)
            Else
                Conn.Execute("delete from   AP_Up_Personal WHERE  Bill_no= '" & (txt_no.Text) & "' and  E_ID= '" & (txtid.Text) & "'")
                Dim aa As String
                aa = "INSERT INTO   AP_Up_Personal (  Bill_no, E_ID, DT_Up, Sections_id, Sections, Department_id, Department, Name_L, Name_E, Phone, Bank_no, SSO_no,percen_old, txtclass_old, txtlevel_old, " & _
               "   txtV_C_old, Level_Clss_Money_old, txtclass, txtlevel, txtV_C, Level_Clss_Money, Tumnang_Money, year_money, txttotal, AGL, Total_remaining, Tax, khongsep,  " & _
       "     txtson, txtson_Money, txtmom, txtMom_mony, txtToltal_All, txtkhor_tok_long, txtAccount_clss_level, type_up_id, type_up,percen,remark, lst_updt, lst_usr, Pc_nm) " & _
