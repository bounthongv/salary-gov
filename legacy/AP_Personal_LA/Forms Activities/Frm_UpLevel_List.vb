Public Class Frm_UpLevel_List
    ...
    Private Sub Button1_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Me.Close()
    End Sub
    Private Sub Frm_CV_List_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Fg1.FormatString = ">ລ/ດ |<ລະຫັດພະນັກງານ |<ຊື່ ແລະ ນາມສະກຸນ              |<ຊື່ (ພາສາອັງກິດ)        |<ເບີໂທລະສັບ    |<ບ່ອນປະຈຳການ   |<ພະແນກ    |^ວັນທີເລີມເຮັດການ|>ຊັ້ນ/ຂັນ |^ວັນທີ່ເລື່ອນຊັ້ນຂັ້ນ|>ຊັ້ນ/ຂັນ  |>ຮັບເງີນເດືອນຕົວຈີງ(%) |>ເງີນເດືອນຕາມ ຊັ້ນ/ຂັ້ນ  |<ບ້ານຢູ່ປະຈຸບັນ |<ເມືອງ             |<ແຂວງ             "

        FG2.FormatString = ">ລ/ດ     |<ລະຫັດພະນັກງານ|<ເລກທີ    |<ເລກທີຂໍ້ຕົກລົງ        |<ເລກທີບັນຊີທຽບຊັ້ນ-ຂັ້ນ    |^ວັນທີ       |<ບ່ອນປະຈຳການ   |<ພະແນກ     |<ປະເພດການເລື່ອນຊັ້ນ-ຂັ້ນ  |<ຊື່ ແລະ ນາມສະກຸນ|>ຊັ້ນ/ຂັນເກົ່າ  |>ເງີນເດືອນຕາມຊັ້ນ/ຂັ້ນເກົ່າ|>ຊັ້ນ/ຂັນໃໝ່|>ເງີນເດືອນຕາມ ຊັ້ນ/ຂັ້ນໃໝ່|>ເງີນອຸດໜູນຕຳແໜ່ງ|>ເງີນອຸດໜູນປີການ|>ລວມເງີນ          |>ເງີນປະກັນສັງຄົມ(8%)|>ລວມເງີນຍັງເຫຼືອ|>ອາກອນເງີນເດືອນ|>ເງີນອຸດໜູນຄ່າຄອງຊີບ|>ຈ/ນລູກ|>ເງີນອຸດໜູນ ລູກ|>ຈ/ນເມຍ|>ເງີນອຸດໜູນ ເມຍ|>ລວມເງີນໄດຕົວຈີງ "
    ...
    Private Sub loaddata()
        ...
        With RSC
            ...
            aa = "SELECT    AP_CV.*, AP_Village.Vl_nm, AP_District.Dt_id, AP_District.Dt_nm, AP_Province.PV_ID, AP_Province.PV_nm, " & _
                   ...
                   "   AP_Province AS AP_Province_1 ON AP_District_1.PV_id = AP_Province_1.PV_ID  where 1=1 and AP_CV.Status_Up=1  " & Sql & " " & shr_section & " " & shr_Department & "  " & shr_job_phuk & "   ORDER BY  AP_CV.E_ID "
            Call LoadRs(aa, RSC)
                ...
                While Not .EOF()
                    Fg1.AddItem(.AbsolutePosition & _
                       ...
                       Chr(9) & Format(CDate(RSC.Fields("DT_Work_now").Value), "dd/MM/yyyy") & _
                                       ...
                                       Chr(9) & Format(CDbl(.Fields("Level_Clss_Money").Value), "##,##0.00") & _
    ...
    Private Sub loaddata_list()
        ...
        With RSC
            Dim aa As String
            aa = "SELECT  * from AP_Up_Personal  where E_ID=N'" & SaleID & "'    ORDER BY Bill_no "
            Call LoadRs(aa, RSC)
                ...
                While Not .EOF()
                    FG2.AddItem(.AbsolutePosition & _
                                 ...
                                 Chr(9) & Trim((RSC.Fields("Bill_no").Value).ToString) & _
                                    Chr(9) & Trim((RSC.Fields("txtkhor_tok_long").Value).ToString) & _
                                  Chr(9) & Trim((RSC.Fields("txtAccount_clss_level").Value).ToString) & _
                                      Chr(9) & Format(CDate(RSC.Fields("DT_Up").Value), "dd/MM/yyyy") & _
                                 Chr(9) & Trim((RSC.Fields("Sections").Value).ToString) & _
                                      Chr(9) & Trim((RSC.Fields("Department").Value).ToString) & _
                                 Chr(9) & Trim((RSC.Fields("type_up").Value).ToString) & _
                                 Chr(9) & Trim((RSC.Fields("Name_L").Value).ToString) & _
                                  Chr(9) & Trim((RSC.Fields("txtV_C_old").Value).ToString) & _
                                     Chr(9) & Format(CDbl(.Fields("Level_Clss_Money_old").Value), "##,##0.00") & _
                                        Chr(9) & Trim((RSC.Fields("txtV_C").Value).ToString) & _
                                           Chr(9) & Format(CDbl(.Fields("Level_Clss_Money").Value), "##,##0.00") & _
                                          Chr(9) & Format(CDbl(.Fields("Tumnang_Money").Value), "##,##0.00") & _
                                           Chr(9) & Format(CDbl(.Fields("year_money").Value), "##,##0.00") & _
                                          Chr(9) & Format(CDbl(.Fields("txttotal").Value), "##,##0.00") & _
                                          Chr(9) & Format(CDbl(.Fields("AGL").Value), "##,##0.00") & _
                                        Chr(9) & Format(CDbl(.Fields("Total_remaining").Value), "##,##0.00") & _
                                               Chr(9) & Format(CDbl(.Fields("Tax").Value), "##,##0.00") & _
                                                Chr(9) & Format(CDbl(.Fields("khongsep").Value), "##,##0.00") & _
                                                 Chr(9) & Format(CDbl(.Fields("txtson").Value), "##,##0.00") & _
                                                Chr(9) & Format(CDbl(.Fields("txtson_Money").Value), "##,##0.00") & _
                                              Chr(9) & Format(CDbl(.Fields("txtmom").Value), "##,##0.00") & _
                                        Chr(9) & Format(CDbl(.Fields("txtMom_mony").Value), "##,##0.00") & _
                                  Chr(9) & Format(CDbl(.Fields("txtToltal_All").Value), "##,##0.00"))
    ...
    Private Sub Button9_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnAddNew.Click
        ...
        EditActive = False
        Frm_Uplevel.MdiParent = FrmAPInvioce
        Frm_Uplevel.WindowState = FormWindowState.Maximized
        Frm_Uplevel.ShowIcon = False
        Frm_Uplevel.Show()
    End Sub
    ...
    Private Sub Button7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDel.Click
        ...
        If MessageBox.Show("ທ່ານຕ້ອງການລຶບລາຍການ ນີ້ແທ້ບໍ່?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Conn.Execute("Delete From AP_Up_Personal Where  E_ID='" & Trim(Fg1.get_TextMatrix(Fg1.Row, 1) & "'"))
        End If
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        With RSC
            Dim sa As String = "SELECT     AP_CV.type_in_id, AP_CV.type_in_nm, AP_CV.DT_strt_work, AP_CV.start_work_ID, AP_CV.start_work, AP_CV.DT_Work_now, AP_CV.cmbclass, " & _
                  "    AP_CV.cmblevel, AP_CV.job_lut_ID, AP_CV.job_lut, AP_CV.Add_Vill_ID, AP_Village.Vl_nm, AP_District.Dt_id, AP_District.Dt_nm, " & _
                   "   AP_Province.PV_ID, AP_Province.PV_nm, AP_Up_Personal.* " & _
                   "       FROM         AP_CV INNER JOIN " & _
                   "   AP_Village ON AP_CV.Add_Vill_ID = AP_Village.Vl_ID INNER JOIN" & _
                     " AP_District ON AP_Village.Dt_id = AP_District.Dt_id INNER JOIN " & _
                 "     AP_Province ON AP_District.PV_id = AP_Province.PV_ID INNER JOIN " & _
                  "    AP_Up_Personal ON AP_CV.E_ID = AP_Up_Personal.E_ID  WHERE 1=1   " & Sql & " " & shr_section & " " & shr_Department & "  " & shr_job_phuk & "   ORDER BY  AP_CV.E_ID "
            Call LoadRs(sa, RSC)
            If .RecordCount = 0 Then MsgBox("Data empty", vbInformation, "Check") : Exit Sub
            Dim Frm As New FrmPreview
            Dim Rpt As New Report_Salary_up_list
    ...
    Private Sub FG2_ClickEvent(ByVal sender As Object, ByVal e As System.EventArgs) Handles FG2.ClickEvent
        ...
        E_ID = FG2.get_TextMatrix(FG2.Row, 1)
