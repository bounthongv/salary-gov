Public Class Deduc_After



    Private Sub Deduc_After_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        'Call Loadlang()
        'SetControlText(Me)
        'ChgChildForm()
    ...
    Private Sub EditData()
        ...
        Call LoadRs("SELECT * FROM List_Deduc_After WHERE Code='" & myID & "' ", rs)
        With rs
            If rs.RecordCount <> 0 Then
                txtid.Text = .Fields("Code").Value.ToString

                txtNm.Text = .Fields("Deduc_After").Value.ToString
    ...
    Private Sub ItemNew()
        ...
        sss = "SELECT TOP 1 Code FROM List_Deduc_After  ORDER BY Code DESC"
        Call LoadRs(sss, cRS)
    ...
    Private Sub SAVE()
        Dim dd As String
        Dim rs As New ADODB.Recordset
        Call LoadRs("SELECT * FROM List_Deduc_After WHERE Code=N'" & txtid.Text & "'", rs)
        If rs.RecordCount = 0 Then
            dd = "INSERT INTO List_Deduc_After(Code, Deduc_After,Money_QTY,Lst_Updt,Lst_Usr,pc_nm) " & _
   ...
   " N'" & MDServerName & "')"
        ...
        Else
            Conn.Execute("DELETE FROM List_Deduc_After WHERE Code=N'" & txtid.Text & "' ")
            dd = "INSERT INTO List_Deduc_After(Code, Deduc_After,Money_QTY,Lst_Updt,Lst_Usr,pc_nm) " & _
              " VALUES(N'" & Trim(txtid.Text.ToString) & "'," & _
             " N'" & Trim(txtNm.Text.ToString) & "'," & _
             " " & CDbl(txtTotal_QTY.Text) & "," & _
     ...
     " N'" & MDServerName & "')"
        ...
        If Chk.Checked = True Then
            Dim ssss As String
            ssss = "UPDATE List_Deduc_After SET chk = '1'" & _
                " WHERE Code =N'" & CStr(txtid.Text) & "'"
            Conn.Execute(ssss)
        Else
            Dim ssss As String
            ssss = "UPDATE List_Deduc_After SET chk = '0'" & _
                " WHERE Code =N'" & CStr(txtid.Text) & "'"
            Conn.Execute(ssss)
        End If
    End Sub
