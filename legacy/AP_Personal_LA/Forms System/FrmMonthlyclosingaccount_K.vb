...
Public Class FrmMonthlyclosingaccount_K
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        With RsCheck
            Call LoadRs("select Open_dt from AP_open_bl_K WHERE month(Open_dt)= '" & Month(DateAdd("m", 1, CDate(Me.lblFromDate.Text & "/" & Me.lblToDate.Text))) & "'  and  year(Open_dt)='" & Year(DateAdd("m", 1, CDate(Me.lblFromDate.Text & "/" & Me.lblToDate.Text))) & "'", RsCheck)
            If .RecordCount <> 0 Then
                MsgBox("Month " & Me.txtMonth.Value & "/" & Me.txtYear.Value & " do to close account", MsgBoxStyle.OkOnly)
                RsCheck = Nothing
                Exit Sub
            Else
                Call LoadRs("select Open_dt from AP_open_bl_K WHERE month(Open_dt)= '" & (Me.lblFromDate.Text) & "'  and  year(Open_dt)='" & (Me.lblToDate.Text) & "' ", RsCheck)
                If .RecordCount = 0 Then
    ...
    Private Sub Clse_Year()
        ...
        Conn.Execute("UPDATE A SET A.Qty=B.Rem_Qty FROM AP_Products_K A, RPT_Inventory B WHERE (A.Pro_ID=B.Pro_ID)")
        'Conn.Execute("DELETE FROM AP_Products_K WHERE Qty=0")
        Conn.Execute("INSERT INTO AP_Clse_Yr_Accnt(Open_Yr, Cat_ID, Pro_ID_Old, Pro_ID_New, Cost, Qty, Lst_Updt, Lst_usr,  PC_nm) " & _
        "SELECT " & Year(txtYear.Value) & ", Cat_ID, Pro_ID, Pro_ID, Cost, Qty, " & Sdate & ", '" & MUserName & "',  '" & MDServerName & "' FROM AP_Products_K ORDER BY Cat_ID, Pro_Cnt")
        Conn.Execute("UPDATE A SET A.Pro_ID=B.Pro_ID_New FROM AP_Products_K A, AP_Clse_Yr_Accnt B WHERE (A.Pro_ID=B.Pro_ID_OLD)")
        Conn.Execute("UPDATE A SET A.Pro_ID=B.Pro_ID_New FROM AP_open_bl_K A, AP_Clse_Yr_Accnt B WHERE (A.Pro_ID=B.Pro_ID_OLD)")
    ...
    Private Sub ClseMnth_Accnt()
        ...
        End If
        Conn.Execute("INSERT INTO AP_open_bl_K (Open_dt, Cat_ID, Pro_id, cost, open_qty, lst_updt, lst_usr, pc_nm) " & _
       "SELECT     '" & Format(ClseDt, "yyyy-MM-dd") & "', " & _
       "Cat_id, Pro_id, cost, Qty, getdate(), '" & Trim(MUserName) & "','" & Trim(MDServerName) & "' FROM AP_Products_K WHERE (Cut_qty = 1)")
        'Conn.Execute("UPDATE AP_Stockin SET Close_accnt=1 WHERE Month(in_dt)=" & Month(txtMonth.Value) & " AND Year(in_dt)=" & Year(txtYear.Value))
        'Conn.Execute("UPDATE AP_SectTurn SET Close_accnt=1 WHERE Month(Turn_dt)=" & Month(txtMonth.Value) & " AND Year(Turn_dt)=" & Year(txtYear.Value))
        'Conn.Execute("UPDATE AP_CustTurn SET Close_accnt=1 WHERE Month(Turn_dt)=" & Month(txtMonth.Value) & " AND Year(Turn_dt)=" & Year(txtYear.Value))
        'Conn.Execute("UPDATE AP_StockReceipt SET Close_accnt=1 WHERE Month(SR_dt)=" & Month(txtMonth.Value) & " AND Year(SR_dt)=" & Year(txtYear.Value))
